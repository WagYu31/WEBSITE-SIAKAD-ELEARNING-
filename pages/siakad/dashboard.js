...
function dosenInfoHeader(user) {
  ...
  const uniqueMK = [...new Set(dj.map(k => k.kode))].length;
  const activeDays = [...new Set(dj.map(k => k.hari))].length;
  const stats = [
    { icon: '\ud83d\udcda', label: 'Mata Kuliah', val: uniqueMK },
    { icon: '\ud83c\udfeb', label: 'Kelas Ajar', val: dj.length },
    { icon: '\ud83d\udcc5', label: 'Hari Aktif', val: activeDays },
    { icon: '\ud83d\udc65', label: 'Mhs. PA', val: DOSEN_BIMBINGAN.length },
  ];
  return `
  <div class="dash-card" style="margin-bottom:20px;overflow:hidden;">
      ...
      <div style="display:flex;align-items:center;gap:20px;flex-wrap:wrap;">
        ...
        <div style="position:relative;flex-shrink:0;">
          <div style="width:68px;height:68px;border-radius:16px;background:linear-gradient(135deg,rgba(255,255,255,0.22),rgba(255,255,255,0.08));border:2px solid rgba(255,255,255,0.35);display:flex;align-items:center;justify-content:center;backdrop-filter:blur(6px);box-shadow:0 4px 16px rgba(0,0,0,0.18);">
            <span style="font-size:1.5rem;color:white;font-weight:800;letter-spacing:-1px;">${getInitials(user.nama)}</span>
          </div>
        ...
        <!-- Name block -->
        <div style="flex:1;min-width:200px;">
          <div style="font-size:1.15rem;font-weight:800;color:white;letter-spacing:-0.3px;">${user.nama}</div>
          <div style="font-size:0.75rem;color:rgba(255,255,255,0.72);margin-top:3px;display:flex;align-items:center;gap:6px;">
            ...
            <span>\u2022</span>
            <span>${user.prodi || 'Administrasi Negara'}</span>
          </div>
        </div>
        <!-- Stats chips -->
        <div style="display:flex;gap:10px;flex-wrap:wrap;">
          ${stats.map(s => `
            <div style="background:rgba(255,255,255,0.14);backdrop-filter:blur(8px);border:1px solid rgba(255,255,255,0.22);border-radius:12px;padding:10px 18px;text-align:center;min-width:72px;">
              ...
              <div style="font-size:1.1rem;font-weight:800;color:white;line-height:1;margin:3px 0;">${s.val}</div>
              <div style="font-size:0.58rem;color:rgba(255,255,255,0.65);font-weight:600;letter-spacing:0.3px;">${s.label}</div>
            </div>
          `).join('')}
        </div>
      </div>
...
function jadwalDosenContent(user) {
  const DAY_CFG = {
    Senin:  { bg:'hsl(213 72% 50%)', lt:'hsl(213 70% 97%)', tx:'hsl(213 72% 30%)', bd:'hsl(213 55% 82%)', dot:'#3b82f6' },
    Selasa: { bg:'hsl(155 58% 40%)', lt:'hsl(155 55% 97%)', tx:'hsl(155 58% 26%)', bd:'hsl(155 45% 78%)', dot:'#22c55e' },
    Rabu:   { bg:'hsl(33 85% 50%)',  lt:'hsl(33 80% 97%)',  tx:'hsl(33 80% 28%)',  bd:'hsl(33 65% 75%)',  dot:'#f97316' },
    Kamis:  { bg:'hsl(272 58% 50%)', lt:'hsl(272 55% 97%)', tx:'hsl(272 58% 32%)', bd:'hsl(272 45% 78%)', dot:'#a855f7' },
    Jumat:  { bg:'hsl(343 62% 50%)', lt:'hsl(343 58% 97%)', tx:'hsl(343 62% 32%)', bd:'hsl(343 50% 78%)', dot:'#ec4899' },
    Sabtu:  { bg:'hsl(183 55% 40%)', lt:'hsl(183 52% 97%)', tx:'hsl(183 55% 26%)', bd:'hsl(183 42% 76%)', dot:'#06b6d4' },
  };
  const DAYS = ['Senin','Selasa','Rabu','Kamis','Jumat','Sabtu'];
  const dj = window._dosenJadwalCache || getDosenJadwal(user);
    ...
    return `
      <tr>
        <td colspan="8" style="padding:0;border:none;">
          <div style="padding:7px 16px;background:${dc.lt};border-left:4px solid ${dc.bg};display:flex;align-items:center;gap:10px;">
            <span style="width:7px;height:7px;border-radius:50%;background:${dc.bg};display:inline-block;"></span>
            <span style="font-weight:800;font-size:0.72rem;color:${dc.tx};letter-spacing:0.8px;">${day.toUpperCase()}</span>
            <span style="font-size:0.66rem;color:${dc.tx};opacity:.6;">${grouped[day].length} kelas</span>
          </div>
        </td>
      </tr>
      ${rows}`;
  ...
  // --- WEEKLY CARDS ---
  const weeklyCards = activeDays.map(day => {
    ...
    return `
      <div style="background:white;border-radius:16px;overflow:hidden;box-shadow:0 2px 14px hsla(215,30%,40%,.08);border:1px solid hsl(215 20% 92%);">
        <div style="background:${dc.bg};padding:12px 18px;display:flex;align-items:center;justify-content:space-between;">
          <div style="display:flex;align-items:center;gap:8px;">
            <span style="font-weight:800;font-size:0.88rem;color:white;">${day}</span>
          </div>
...
async function initPertemuanCards(kelasIdx, outerDiv) {
  ...
  outerDiv.querySelectorAll('.btn-buka-kelas').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      // Inject premium modal
      const modalId = 'modalBukaKelas';
      document.getElementById(modalId)?.remove();
      // Inject keyframe if not present
      if (!document.getElementById('_bukaKelasStyle')) {
        const st = document.createElement('style');
        st.id = '_bukaKelasStyle';
        st.textContent = `
      ...
      overlay.id = modalId;
      overlay.style.cssText = 'position:fixed;inset:0;background:rgba(15,23,42,0.55);z-index:9999;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(6px);padding:16px;';
      const pct = Math.round((n-1)/14*100);
      const dateStr2 = (dates[n-1] ? formatTanggalShort(dates[n-1]) : '-');
      const modeIconMap = {offline:'🏢',online:'🖥️',hybrid:'🔄'};
      overlay.innerHTML = `
          ...
          <div style="background:linear-gradient(135deg,hsl(152 55% 36%),hsl(165 50% 46%));padding:24px 24px 20px;position:relative;overflow:hidden;">
            ...
            <div style="display:flex;align-items:center;gap:14px;position:relative;">
              ...
              <div style="flex:1;min-width:0;">
                ...
                <div style="font-size:1.1rem;font-weight:800;color:white;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${kelas.nama}</div>
                <div style="display:flex;align-items:center;gap:8px;margin-top:6px;flex-wrap:wrap;">
                  <span style="background:rgba(255,255,255,0.15);backdrop-filter:blur(4px);color:rgba(255,255,255,0.9);font-size:0.65rem;font-weight:700;padding:3px 10px;border-radius:20px;border:1px solid rgba(255,255,255,0.2);">${kode}</span>
                  <span style="background:rgba(255,255,255,0.15);backdrop-filter:blur(4px);color:rgba(255,255,255,0.9);font-size:0.65rem;font-weight:700;padding:3px 10px;border-radius:20px;border:1px solid rgba(255,255,255,0.2);">Kelas ${klsName}</span>
                  <span style="background:rgba(255,255,255,0.15);backdrop-filter:blur(4px);color:rgba(255,255,255,0.9);font-size:0.65rem;font-weight:700;padding:3px 10px;border-radius:20px;border:1px solid rgba(255,255,255,0.2);">Pertemuan ${n}/14</span>
                </div>
              </div>
            </div>
            <!-- Progress bar -->
            <div style="margin-top:16px;position:relative;">
              <div style="display:flex;justify-content:space-between;margin-bottom:5px;">
                ...
                <span style="font-size:0.62rem;color:rgba(255,255,255,0.85);font-weight:700;">${n-1}/14 selesai</span>
              </div>
              <div style="background:rgba(255,255,255,0.2);border-radius:4px;height:5px;overflow:hidden;">
                <div style="height:100%;width:${pct}%;background:rgba(255,255,255,0.8);border-radius:4px;transition:width .4s;"></div>
              </div>
            </div>
          </div>
          <!-- Session info strip -->
          <div style="background:hsl(152 30% 96%);border-bottom:1px solid hsl(152 25% 90%);padding:12px 24px;display:flex;gap:16px;align-items:center;">
            <div style="display:flex;align-items:center;gap:6px;">
              ...
              <div><div style="font-size:0.6rem;color:hsl(215 15% 55%);font-weight:600;text-transform:uppercase;letter-spacing:.4px;">Tanggal</div>
                <div style="font-size:0.75rem;font-weight:700;color:hsl(215 25% 25%);">${dateStr2}</div></div>
            </div>
            ...
            <div style="display:flex;align-items:center;gap:6px;">
              ...
              <div><div style="font-size:0.6rem;color:hsl(215 15% 55%);font-weight:600;text-transform:uppercase;letter-spacing:.4px;">Hari</div>
                <div style="font-size:0.75rem;font-weight:700;color:hsl(215 25% 25%);">${kelas.hari}</div></div>
            </div>
            <div style="width:1px;height:28px;background:hsl(152 25% 85%);"></div>
            <div style="display:flex;align-items:center;gap:6px;">
              <span style="font-size:0.95rem;">${modeIconMap[curMode]||'🏢'}</span>
              <div><div style="font-size:0.6rem;color:hsl(215 15% 55%);font-weight:600;text-transform:uppercase;letter-spacing:.4px;">Mode Default</div>
                <div style="font-size:0.75rem;font-weight:700;color:hsl(215 25% 25%);">${curMode.charAt(0).toUpperCase()+curMode.slice(1)}</div></div>
            </div>
          </div>
          <!-- Form body -->
            ...
            <div>
              ...
              <div style="display:flex;gap:8px;">
                ${['offline','online','hybrid'].map(m => `
                  <label class="bk-mode-lbl" style="flex:1;display:flex;flex-direction:column;align-items:center;gap:4px;padding:10px 6px;border:1.5px solid hsl(215 20% 87%);border-radius:10px;background:white;font-size:0.68rem;font-weight:700;color:hsl(215 20% 45%);${m===curMode?'border-color:hsl(152 55% 45%);background:hsl(152 55% 96%);color:hsl(152 45% 30%);':''}">
                    <input type="radio" name="modePertemuan" value="${m}" ${m===curMode?'checked':''} style="position:absolute;opacity:0;pointer-events:none;">
                    <span style="font-size:1.2rem;">${m==='offline'?'🏢':m==='online'?'🖥️':'🔄'}</span>
      ...
      // Mode radio click handler
      overlay.querySelectorAll('.bk-mode-lbl').forEach(lbl => {
        lbl.addEventListener('click', () => {
          overlay.querySelectorAll('.bk-mode-lbl').forEach(l => { l.style.borderColor='hsl(215 20% 87%)'; l.style.background='white'; l.style.color='hsl(215 20% 45%)'; });
          lbl.style.borderColor='hsl(152 55% 45%)'; lbl.style.background='hsl(152 55% 96%)'; lbl.style.color='hsl(152 45% 30%)';
          lbl.querySelector('input').checked = true;
        });
      });
      // Catatan char counter
      document.getElementById('inputCatatanPertemuan').addEventListener('input', function() {
...
function initInlinePertemuanEditor(entry) {
  ...
  // ---- Create / reset the section container in DOM ----
  const formArea = document.getElementById('jadwalFormArea');
  if (!formArea) return;
  ...
  // Build & append fresh section
  const section = document.createElement('div');
  section.id = 'inlinePrtSection';
  section.className = 'dash-card';
  section.style.cssText = 'margin-top:16px;overflow:hidden;border:2px solid hsl(170 55% 68%);';
  ...
  content.innerHTML = `
    ...
    <div class="stat-grid pmb-stat-grid" style="margin-bottom:20px;">
      <div class="pmb-glow-card" style="background:linear-gradient(135deg,hsl(215 65% 50%),hsl(230 70% 58%));">
        ...
        <div class="pmb-gc-info"><div class="pmb-gc-value">${stats.total_pendaftar}</div><div class="pmb-gc-label">Total Pendaftar</div></div>
      ...
      <div class="pmb-glow-card" style="background:linear-gradient(135deg,hsl(38 75% 52%),hsl(28 80% 48%));">
        ...
        <div class="pmb-gc-info"><div class="pmb-gc-value">${stats.total_proses}</div><div class="pmb-gc-label">Proses</div></div>
        <div class="pmb-gc-ring"></div>
      </div>
      <div class="pmb-glow-card" style="background:linear-gradient(135deg,hsl(150 55% 42%),hsl(160 60% 38%));">
        ...
        <div class="pmb-gc-info"><div class="pmb-gc-value">${stats.total_diterima}</div><div class="pmb-gc-label">Diterima</div></div>
        <div class="pmb-gc-ring"></div>
      </div>
      <div class="pmb-glow-card" style="background:linear-gradient(135deg,hsl(0 60% 55%),hsl(350 65% 48%));">
        ...
        <div class="pmb-gc-info"><div class="pmb-gc-value">${stats.total_ditolak}</div><div class="pmb-gc-label">Ditolak</div></div>
        <div class="pmb-gc-ring"></div>
      </div>
    </div>
    ...
    <table class="sch-table" style="font-size:0.82rem;">
      ...
      <tbody>
        ${registrations.map(r => `<tr class="pmb-tr" data-id="${r.id}" style="cursor:pointer;${_pmbSelected.has(r.id)?'background:hsl(215 80% 97%);':''}">
          ...
          ">${(r.metode||'').toLowerCase()==='online' ? '🌐 Online' : '🏢 Offline'}</span></td>
          <td><span style="display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border-radius:20px;font-size:0.7rem;font-weight:700;letter-spacing:.03em;white-space:nowrap;
            ${(()=>{const s=(r.status||'').toLowerCase();return s==='diterima'?'background:hsl(142 65% 90%);color:hsl(142 60% 28%);border:1px solid hsl(142 50% 78%);':s==='ditolak'?'background:hsl(0 70% 92%);color:hsl(0 65% 38%);border:1px solid hsl(0 55% 80%);':s==='proses'?'background:hsl(225 70% 92%);color:hsl(225 65% 40%);border:1px solid hsl(225 55% 80%);':'background:hsl(38 80% 91%);color:hsl(38 70% 36%);border:1px solid hsl(38 60% 78%);';})()}
          ">${(()=>{const s=(r.status||'').toLowerCase();return s==='diterima'?'✅ Diterima':s==='ditolak'?'❌ Ditolak':s==='proses'?'🔄 Proses':'⏳ Menunggu';})()}</span></td>
      ...
      <div style="margin-bottom:14px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
          ${(()=>{
            ...
            const berkas = [
              { key:'file_pasfoto',   label:'Pas Foto',       icon:'🖼️' },
              { key:'file_ktp',       label:'KTP/Kartu ID',   icon:'🪪' },
              { key:'file_ijazah',    label:'Ijazah/SKHUN',   icon:'📜' },
              { key:'file_rapor',     label:'Rapor',          icon:'📋' },
              { key:'file_surat_sehat', label:'Surat Sehat',  icon:'🏥' },
            ];
            return berkas.map(b => {
              ...
              return `<div style="background:hsl(215 20% 97%);border:1px solid hsl(215 20% 90%);border-radius:10px;padding:10px;">
                <p style="font-size:0.68rem;color:var(--text-muted);font-weight:600;text-transform:uppercase;margin-bottom:6px;">${b.icon} ${b.label}</p>
                ${path
                  ? (isImg
                    ? `<a href="${BASE+path}" target="_blank"><img src="${BASE+path}" alt="${b.label}" style="width:100%;max-height:80px;object-fit:cover;border-radius:6px;cursor:pointer;"></a>`
                    : isPdf
                    ? `<a href="${BASE+path}" target="_blank" style="display:flex;align-items:center;gap:6px;font-size:0.78rem;font-weight:600;color:hsl(215 60% 45%);text-decoration:none;"><svg width='14' height='14' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2'><path d='M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z'/><polyline points='14,2 14,8 20,8'/></svg> Buka PDF</a>`
                    : `<span style='font-size:0.75rem;color:hsl(215 55% 48%);'>📎 Lihat Berkas</span>`
                  )
                  : `<span style="font-size:0.72rem;color:hsl(38 60% 50%);font-style:italic;">⚠️ Belum diupload</span>`
                }
              </div>`;
            }).join('');
          })()}
                      ...
                      <tr style="border-bottom:1px solid hsl(215 15% 90%);">
                        <td style="padding:10px 14px;">
                          <span style="display:inline-block;padding:2px 8px;border-radius:6px;font-size:0.72rem;font-weight:600;margin-bottom:2px;
                            background:${p.type === 'pmb' ? 'hsl(215 80% 92%)' : 'hsl(145 60% 92%)'};
                            color:${p.type === 'pmb' ? 'hsl(215 70% 40%)' : 'hsl(145 60% 30%)'};">
                            ${p.type === 'pmb' ? '📝 PMB' : '📚 Semester'}
                          </span><br>
                          <span style="font-size:0.78rem;color:var(--text-muted);">${p.jenis || p.order_id}</span>
                        </td>
                        <td style="padding:10px 14px;font-weight:600;">${formatRp(p.jumlah)}</td>
                        <td style="padding:10px 14px;">${p.metode_bayar === 'online' ? '🌐 Online' : p.metode_bayar === 'cash' ? '💵 Cash' : p.metode_bayar || '-'}</td>
