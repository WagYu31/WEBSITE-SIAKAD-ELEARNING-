...
`),n=new Blob([t],{type:`text/csv`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=`PMB_Data_${new Date().toISOString().slice(0,10)}.csv`,i.click(),URL.revokeObjectURL(r)}async function It(e,t){try{let n,r;switch(e){case`create-account`:if(n=await fetch(`${R}/account/create`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({registration_id:parseInt(t.id)})}),r=await n.json(),n.ok){let e=r.password;alert(e?`✅ Akun dibuat!\n\nNIM: ${r.nim}\nPassword: ${e}\nEmail: ${r.email}`:`ℹ️ Akun sudah dibuat sebelumnya.\n\nNIM: ${r.nim}\nEmail: ${r.email}\n\nPassword hanya ditampilkan saat pertama kali dibuat.`)}else alert(`❌ `+(r.error||`Gagal membuat akun`));break;case`validate`:let e=await fetch(`${R}/account/${t.id}`);if(!e.ok){alert(`❌ Akun belum dibuat. Buat akun terlebih dahulu.`);return}if((await e.json()).is_validated){alert(`ℹ️ Akun sudah divalidasi sebelumnya.`);return}let i=await fetch(`${R}/account/${t.id}/validate`,{method:`PUT`});r=await i.json(),i.ok?(alert(`✅ `+r.message),B()):alert(`❌ `+(r.error||`Gagal validasi`));break;case`confirm-pay`:let a=await fetch(`${R}/payment`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({registration_id:parseInt(t.id),metode_bayar:`cash`,jumlah:35e4})});if(a.ok){let e=await a.json();(await fetch(`${R}/payment/${e.id}/confirm`,{method:`PUT`})).ok&&(alert(`✅ Pembayaran cash dikonfirmasi!`),B())}else{let e=await a.json();e.error&&e.error.includes(`sudah`)?alert(`ℹ️ Pembayaran sudah tercatat.`):alert(`❌ `+(e.error||`Gagal`))}break;case`edit`:Rt(parseInt(t.id));return;case`delete`:let o=z.find(e=>e.id===parseInt(t.id));if(!o||!confirm(`⚠️ Hapus data pendaftaran?\n\n${o.nama} (${o.no_pendaftaran})\n\nSemua data terkait (akun, pembayaran) juga akan dihapus.`))return;n=await fetch(`${R}/registration/${t.id}`,{method:`DELETE`}),r=await n.json(),n.ok?(alert(`✅ `+r.message),B()):alert(`❌ `+(r.error||`Gagal menghapus`));break}}catch(e){alert(`❌ Error: `+e.message)}}function Lt(){let e=document.getElementById(`pmbMgmtContent`);e&&(e.innerHTML=`
    ...
    </div>`}function Vt(){Ht(),document.getElementById(`mhsSearch`)?.addEventListener(`input`,Wt),document.getElementById(`mhsFilterProdi`)?.addEventListener(`change`,Wt),document.getElementById(`mhsFilterStatus`)?.addEventListener(`change`,Wt)}async function Ht(){try{let e=await fetch(`${zt}/registrations`);if(!e.ok)throw Error(`Gagal memuat data`);let t=await e.json();W=(Array.isArray(t)?t:t.data||[]).filter(e=>e.status===`diterima`).map((e,t)=>({...e,nim:e.nim||`20260${String(t+1).padStart(4,`0`)}`,angkatan:e.angkatan||(e.created_at?new Date(e.created_at).getFullYear():2026),semester:e.semester||1,status_mhs:e.status_mhs||`aktif`})),Ut(),Gt(W)}catch{W=te.map(e=>({...e})),Ut(),Gt(W)}}function Ut(){let e=W.length,t=W.filter(e=>e.status_mhs===`aktif`).length,n=W.filter(e=>e.status_mhs===`cuti`).length,r=W.filter(e=>e.status_mhs===`lulus`).length,i=e=>document.getElementById(e);i(`mhsTotal`)&&(i(`mhsTotal`).textContent=e),i(`mhsAktif`)&&(i(`mhsAktif`).textContent=t),i(`mhsCuti`)&&(i(`mhsCuti`).textContent=n),i(`mhsLulus`)&&(i(`mhsLulus`).textContent=r)}function Wt(){let e=(document.getElementById(`mhsSearch`)?.value||``).toLowerCase(),t=document.getElementById(`mhsFilterProdi`)?.value||``,n=document.getElementById(`mhsFilterStatus`)?.value||``;Gt(W.filter(r=>{let i=!e||(r.nama||``).toLowerCase().includes(e)||(r.nim||``).toLowerCase().includes(e)||(r.nik||``).includes(e),a=!t||r.prodi_pilihan===t,o=!n||r.status_mhs===n;return i&&a&&o}))}function Gt(e){let t=document.getElementById(`mhsTableContainer`),n=document.getElementById(`mhsCount`);if(t){if(e.length===0){t.innerHTML=`<div style="text-align:center;padding:32px;">
      <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="var(--gray-300)" stroke-width="1.5" style="margin:0 auto 12px;display:block;"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>
