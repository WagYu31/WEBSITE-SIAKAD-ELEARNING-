...
Setelah selesai bayar, refresh halaman ini.`)}}break}case`edit`:zt(t.id);return;case`delete`:let i=z.find(e=>String(e.id)===String(t.id));if(!i||!confirm(`⚠️ Hapus data pendaftaran?\n\n${i.nama} (${i.no_pendaftaran})\n\nSemua data terkait (akun, pembayaran) juga akan dihapus.`))return;n=await fetch(`${R}/registration/${t.id}`,{method:`DELETE`}),r=await n.json(),n.ok?(alert(`✅ `+r.message),B()):alert(`❌ `+(r.error||`Gagal menghapus`));break}}catch(e){alert(`❌ Error: `+e.message)}}function Rt(){let e=document.getElementById(`pmbMgmtContent`);e&&(e.innerHTML=`
    ...
    <form id="mhsEditForm" style="max-height:60vh;overflow-y:auto;padding-right:4px;">
      ...
      <div class="off-section" style="margin-bottom:16px;">
        ...
        <p style="font-size:0.78rem;color:var(--text-muted);margin-bottom:12px;">File sudah terupload ditampilkan di bawah. Klik <strong>Ganti</strong> untuk upload ulang.</p>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
          ${(()=>{let t=`https://stiabayuanggajobs.online/api/`;return[{key:`file_ijazah`,name:`file_ijazah`,label:`Ijazah SMA/SMK/MA`,icon:`📜`,accept:`.pdf,.jpg,.jpeg,.png`},{key:`file_ktp`,name:`file_ktp`,label:`KTP / KK`,icon:`🪪`,accept:`.pdf,.jpg,.jpeg,.png`},{key:`file_pasfoto`,name:`file_pasfoto`,label:`Pas Foto 3×4`,icon:`📷`,accept:`.jpg,.jpeg,.png`}].map(n=>{let r=e[n.key],i=r&&/\.(jpg|jpeg|png|webp)$/i.test(r),a=r&&/\.pdf$/i.test(r),o=r?i?`<a href="${t+r}" target="_blank"><img src="${t+r}" alt="${n.label}" style="width:100%;max-height:70px;object-fit:cover;border-radius:6px;cursor:pointer;margin-bottom:4px;"></a>`:a?`<a href="${t+r}" target="_blank" style="display:flex;align-items:center;gap:5px;font-size:0.72rem;font-weight:600;color:hsl(215 60% 45%);text-decoration:none;margin-bottom:4px;">📋 Buka PDF</a>`:`<span style="font-size:0.72rem;color:hsl(215 55% 48%);">📎 Lihat Berkas</span>`:`<span style="font-size:0.72rem;color:hsl(38 65% 50%);font-style:italic;">⚠️ Belum diupload</span>`,s=r?`<span style="background:hsl(145 55% 90%);color:hsl(145 50% 30%);border-radius:12px;padding:1px 8px;font-size:0.65rem;font-weight:700;">✅ Ada</span>`:`<span style="background:hsl(38 90% 90%);color:hsl(38 60% 35%);border-radius:12px;padding:1px 8px;font-size:0.65rem;font-weight:700;">⚠️ Kosong</span>`;return`<div style="background:hsl(215 20% 97%);border:1px solid hsl(215 20% 90%);border-radius:10px;padding:10px;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                  <span style="font-size:0.7rem;color:var(--text-muted);font-weight:600;text-transform:uppercase;">${n.icon} ${n.label}</span>
                  ${s}
                </div>
