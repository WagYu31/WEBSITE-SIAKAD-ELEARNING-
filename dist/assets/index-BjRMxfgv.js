(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e={},t={},n=null;function r(n,r,i=``){e[n]=r,t[n]=i}function i(e){window.location.hash=e}function a(e){n=document.getElementById(e),window.addEventListener(`hashchange`,o),window.addEventListener(`load`,o),window.location.hash?o():window.location.hash=`#/`}async function o(){let r=window.location.hash.slice(1)||`/`,i=e[r];if(!i){for(let[t,n]of Object.entries(e))if(t.includes(`:`)){let e=RegExp(`^`+t.replace(/:[^/]+/g,`([^/]+)`)+`$`);if(r.match(e)){i=n;break}}}if(i&&n){n.style.opacity=`0`,n.style.transform=`translateY(10px)`,await new Promise(e=>setTimeout(e,150));try{await i(n)}catch(e){console.error(`Route error:`,e),n.innerHTML=`
  ...
  <div class="dash-card" style="margin-bottom:20px;overflow:hidden;">
    ...
    </div>`}async function Pt(){document.querySelectorAll(`.pmb-mgmt-btn`).forEach(e=>{e.addEventListener(`click`,()=>{document.querySelectorAll(`.pmb-mgmt-btn`).forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),e.dataset.tab===`list`?B():e.dataset.tab===`add`&&Jt()})}),document.getElementById(`closeDetailModal`)?.addEventListener(`click`,()=>{document.getElementById(`pmbDetailModal`).style.display=`none`}),document.getElementById(`pmbDetailModal`)?.addEventListener(`click`,e=>{e.target.id===`pmbDetailModal`&&(e.target.style.display=`none`)}),B()}var z=[];async function B(){let e=document.getElementById(`pmbMgmtContent`);if(e){e.innerHTML=`<div style="text-align:center;padding:24px;">
    ...
    </div>`;let r=()=>{V=1,Lt()};document.getElementById(`pmbSearch`)?.addEventListener(`input`,r),document.getElementById(`pmbFilterStatus`)?.addEventListener(`change`,r),document.getElementById(`pmbFilterProdi`)?.addEventListener(`change`,r),document.getElementById(`pmbDateFrom`)?.addEventListener(`change`,r),document.getElementById(`pmbDateTo`)?.addEventListener(`change`,r),document.getElementById(`pmbClearDate`)?.addEventListener(`click`,()=>{document.getElementById(`pmbDateFrom`).value=``,document.getElementById(`pmbDateTo`).value=``,r()}),document.getElementById(`pmbExportBtn`)?.addEventListener(`click`,()=>Kt(z)),document.getElementById(`pmbBulkValidate`)?.addEventListener(`click`,Vt),document.getElementById(`pmbBulkAccount`)?.addEventListener(`click`,Ht),document.getElementById(`pmbBulkDelete`)?.addEventListener(`click`,Ut),document.getElementById(`pmbBulkCancel`)?.addEventListener(`click`,()=>{U.clear(),Bt(),Lt()}),Lt()}function Lt(){let e=(document.getElementById(`pmbSearch`)?.value||``).toLowerCase(),t=document.getElementById(`pmbFilterStatus`)?.value||``,n=document.getElementById(`pmbFilterProdi`)?.value||``,r=document.getElementById(`pmbDateFrom`)?.value||``,i=document.getElementById(`pmbDateTo`)?.value||``,a=z.filter(a=>{let o=!e||(a.nama||``).toLowerCase().includes(e)||(a.nik||``).includes(e)||(a.no_pendaftaran||``).toLowerCase().includes(e),s=!t||(a.status||``).toLowerCase()===t.toLowerCase(),c=!n||a.prodi_pilihan===n,l=!0;if(r||i){let e=a.created_at?a.created_at.slice(0,10):``;r&&e<r&&(l=!1),i&&e>i&&(l=!1)}return o&&s&&c&&l}),{key:o,dir:s}=H;a.sort((e,t)=>{let n=e[o]||``,r=t[o]||``;return typeof n==`string`&&(n=n.toLowerCase()),typeof r==`string`&&(r=r.toLowerCase()),n<r?s===`asc`?-1:1:n>r?s===`asc`?1:-1:0});let c=a.length,l=Math.max(1,Math.ceil(c/Ft));V>l&&(V=l);let u=(V-1)*Ft,d=a.slice(u,u+Ft);document.getElementById(`pmbTableWrapper`).innerHTML=zt(d),document.querySelectorAll(`.pmb-act`).forEach(function(e){e.addEventListener(`click`,function(t){t.stopPropagation(),t.preventDefault(),qt(e.getAttribute(`data-act`),{id:e.getAttribute(`data-rid`),email:e.getAttribute(`data-email`)||``,prodi:e.getAttribute(`data-prodi`)||``})})}),document.querySelectorAll(`.pmb-tr`).forEach(function(e){e.addEventListener(`click`,function(t){if(!(t.target.closest(`.pmb-act`)||t.target.closest(`input`))){var n=e.getAttribute(`data-id`),r=z.find(function(e){return String(e.id)===String(n)});r&&Gt(r)}})}),document.getElementById(`pmbResultCount`).textContent=c>0?`Menampilkan ${u+1}–${Math.min(u+Ft,c)} dari ${c} pendaftar`:`Tidak ada data`,Rt(l),Wt()}function Rt(e){let t=document.getElementById(`pmbPagination`);if(!t||e<=1){t&&(t.innerHTML=``);return}let n=e=>`style="padding:4px 10px;border-radius:6px;border:1px solid ${e?`var(--primary-500)`:`var(--gray-200)`};background:${e?`var(--primary-500)`:`#fff`};color:${e?`#fff`:`var(--text-primary)`};cursor:pointer;font-size:0.75rem;font-weight:600;"`,r=`<button ${n(!1)} ${V<=1?`disabled`:``} data-pg="${V-1}">‹</button>`,i=Math.max(1,V-2),a=Math.min(e,i+4);a-i<4&&(i=Math.max(1,a-4));for(let e=i;e<=a;e++)r+=`<button ${n(e===V)} data-pg="${e}">${e}</button>`;r+=`<button ${n(!1)} ${V>=e?`disabled`:``} data-pg="${V+1}">›</button>`,t.innerHTML=r,t.querySelectorAll(`button[data-pg]`).forEach(t=>t.addEventListener(`click`,()=>{let n=parseInt(t.dataset.pg);n>=1&&n<=e&&(V=n,Lt())}))}function zt(e){if(e.length===0)return`<div style="text-align:center;padding:32px;">
    ...
    </table>`}function Bt(){let e=document.getElementById(`pmbBulkBar`),t=document.getElementById(`pmbBulkCount`);e&&(U.size>0?(e.style.display=`flex`,t&&(t.textContent=`${U.size} pendaftar dipilih`)):e.style.display=`none`)}async function Vt(){if(!confirm(`Validasi ${U.size} pendaftar sekaligus?`))return;let e=0,t=0;for(let n of U)try{(await fetch(`${R}/account/${n}/validate`,{method:`PUT`})).ok?e++:t++}catch{t++}alert(`✅ Berhasil: ${e}\n❌ Gagal: ${t}`),U.clear(),B()}async function Ht(){if(!confirm(`Buat akun untuk ${U.size} pendaftar sekaligus?`))return;let e=0,t=0;for(let n of U)try{(await fetch(`${R}/account/create`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({registration_id:n})})).ok?e++:t++}catch{t++}alert(`✅ Berhasil: ${e}\n❌ Gagal: ${t}`),U.clear(),B()}async function Ut(){if(!confirm(`⚠️ HAPUS ${U.size} pendaftar?\n\nAksi ini tidak bisa dibatalkan!`))return;let e=0,t=0;for(let n of U)try{(await fetch(`${R}/registration/${n}`,{method:`DELETE`})).ok?e++:t++}catch{t++}alert(`✅ Terhapus: ${e}\n❌ Gagal: ${t}`),U.clear(),B()}function Wt(){window.__pmbClickHandler&&document.removeEventListener(`click`,window.__pmbClickHandler,!0),window.__pmbClickHandler=function(e){let t=e.target.closest(`.pmb-act`);if(t&&t.dataset.act&&t.dataset.rid){e.preventDefault(),e.stopPropagation(),qt(t.dataset.act,{id:t.dataset.rid,email:t.dataset.email||``,prodi:t.dataset.prodi||``});return}let n=e.target.closest(`.pmb-tr`);if(n&&!e.target.closest(`.pmb-act`)&&!e.target.closest(`input[type="checkbox"]`)){let e=parseInt(n.dataset.id),t=z.find(t=>t.id===e);t&&Gt(t)}},document.addEventListener(`click`,window.__pmbClickHandler,!0),document.querySelectorAll(`.sortable-th`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.sort;H.key===t?H.dir=H.dir===`asc`?`desc`:`asc`:H={key:t,dir:`asc`},Lt()})}),document.getElementById(`pmbSelectAll`)?.addEventListener(`change`,e=>{document.querySelectorAll(`.pmb-chk`).forEach(t=>{let n=parseInt(t.dataset.id);e.target.checked?U.add(n):U.delete(n),t.checked=e.target.checked}),Bt()}),document.querySelectorAll(`.pmb-chk`).forEach(e=>{e.addEventListener(`change`,()=>{let t=parseInt(e.dataset.id);e.checked?U.add(t):U.delete(t),Bt()})})}async function Gt(e){let t=document.getElementById(`pmbDetailModal`),n=document.getElementById(`pmbDetailContent`);if(!t||!n)return;let r=t.querySelector(`h3`);r&&(r.innerHTML=`👁️ Detail Pendaftar`);let i=t.querySelector(`div`);i&&(i.style.borderTop=`4px solid hsl(210 70% 55%)`);let a={diterima:`badge-success`,ditolak:`badge-danger`,proses:`badge-primary`,menunggu:`badge-warning`},o=e=>e||`<span style="color:var(--danger-400);font-style:italic;">— kosong</span>`,s=e=>e?`✅`:`❌`,c=e=>e?new Date(e).toLocaleDateString(`id-ID`,{day:`numeric`,month:`long`,year:`numeric`}):`-`,l=e=>e?new Date(e).toLocaleString(`id-ID`,{day:`2-digit`,month:`short`,year:`numeric`,hour:`2-digit`,minute:`2-digit`}):`-`,u=[`nik`,`nama`,`email`,`telepon_1`,`prodi_pilihan`,`asal_sekolah`,`alamat`,`tempat_lahir`,`tanggal_lahir`,`gender`],d=u.filter(t=>e[t]&&String(e[t]).trim()).length,f=Math.round(d/u.length*100),p=f===100?`hsl(145 60% 45%)`:f>=70?`hsl(38 90% 50%)`:`hsl(0 70% 55%)`;n.innerHTML=`
    ...
    </div>`,n.querySelectorAll(`.mgmt-action-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation(),qt(e.dataset.action,e.dataset)})}),t.style.display=`flex`;let m=n.querySelector(`#pmbAccSection`);try{let t=await fetch(`${R}/account/${e.id}`);if(t.ok){let n=await t.json(),r=n.plain_password,i=`pmbPwdToggle_`+e.id,a=n.is_validated?`<span style="color:hsl(145 55% 40%);font-weight:600;">✅ Tervalidasi</span> <span style="font-size:0.72rem;color:var(--text-muted);">oleh ${n.validated_by||`BAP`} · ${l(n.validated_at)}</span>`:`<span style="color:hsl(38 75% 45%);font-weight:600;">⏳ Belum Divalidasi</span>`,o=l(n.created_at);m.innerHTML=`
...
Setelah selesai bayar, refresh halaman ini.`)}}break}case`edit`:Yt(t.id);return;case`delete`:let i=z.find(e=>String(e.id)===String(t.id));if(!i||!confirm(`⚠️ Hapus data pendaftaran?\n\n${i.nama} (${i.no_pendaftaran})\n\nSemua data terkait (akun, pembayaran) juga akan dihapus.`))return;n=await fetch(`${R}/registration/${t.id}`,{method:`DELETE`}),r=await n.json(),n.ok?(alert(`✅ `+r.message),B()):alert(`❌ `+(r.error||`Gagal menghapus`));break}}catch(e){alert(`❌ Error: `+e.message)}}function Jt(){let e=document.getElementById(`pmbMgmtContent`);e&&(e.innerHTML=`
    ...
    </div>`}function Qt(){$t(),document.getElementById(`mhsSearch`)?.addEventListener(`input`,tn),document.getElementById(`mhsFilterProdi`)?.addEventListener(`change`,tn),document.getElementById(`mhsFilterStatus`)?.addEventListener(`change`,tn)}async function $t(){try{let e=await fetch(`${Xt}/registrations`);if(!e.ok)throw Error(`Gagal memuat data`);let t=await e.json(),n=Array.isArray(t)?t:t.data||[];if(n.length===0){W=[],en(),nn(W);return}let r=await Promise.allSettled(n.map(e=>fetch(`${Xt}/account/${e.id}`).then(e=>e.ok?e.json():null).catch(()=>null))),i={menunggu:`menunggu`,proses:`proses`,diterima:`aktif`,ditolak:`ditolak`};W=n.map((e,t)=>{let n=r[t],a=n?.status===`fulfilled`?n.value:null,o=a?.nim||e.nim||`PMB${String(e.id).padStart(4,`0`)}`,s=e.created_at?new Date(e.created_at).getFullYear():2026,c=(e.status||`menunggu`).toLowerCase(),l=i[c]||c;return{...e,nim:o,email:a?.email||e.email||``,angkatan:s,semester:e.semester||1,status_mhs:l,prodi:e.prodi_pilihan||e.jurusan_pilihan||`-`}}),en(),nn(W)}catch(e){W=[],en(),nn(W);let t=document.getElementById(`mhsTableContainer`);t&&(t.innerHTML=`<div style="text-align:center;padding:48px;color:hsl(215 15% 55%);">❌ Gagal memuat data: ${e.message}</div>`)}}function en(){let e=W.length,t=W.filter(e=>[`aktif`,`diterima`].includes(e.status_mhs)).length,n=W.filter(e=>[`menunggu`,`proses`].includes(e.status_mhs)).length,r=W.filter(e=>e.status_mhs===`lulus`).length,i=e=>document.getElementById(e);i(`mhsTotal`)&&(i(`mhsTotal`).textContent=e),i(`mhsAktif`)&&(i(`mhsAktif`).textContent=t),i(`mhsCuti`)&&(i(`mhsCuti`).textContent=n),i(`mhsLulus`)&&(i(`mhsLulus`).textContent=r);let a=i(`mhsCuti`)?.nextElementSibling;a&&(a.textContent=`Proses`)}function tn(){let e=(document.getElementById(`mhsSearch`)?.value||``).toLowerCase(),t=document.getElementById(`mhsFilterProdi`)?.value||``,n=document.getElementById(`mhsFilterStatus`)?.value||``;nn(W.filter(r=>{let i=!e||(r.nama||``).toLowerCase().includes(e)||(r.nim||``).toLowerCase().includes(e)||(r.nik||``).includes(e)||(r.prodi||``).toLowerCase().includes(e.toLowerCase()),a=!t||r.prodi_pilihan===t||r.prodi===t||r.jurusan_pilihan===t,o=!n||r.status_mhs===n;return i&&a&&o}))}function nn(e){let t=document.getElementById(`mhsTableContainer`),n=document.getElementById(`mhsCount`);if(!t)return;if(e.length===0){t.innerHTML=`<div style="text-align:center;padding:48px 32px;">
    ...
    <div style="display:flex;gap:8px;margin-top:16px;padding-top:14px;border-top:1px solid hsl(215 15% 93%);flex-wrap:wrap;">
      ...
      <button onclick="if(confirm('⚠️ Hapus data mahasiswa ${e.nama}?\\n\\nTindakan ini tidak dapat dibatalkan.')) { document.getElementById('mhsModal').style.display='none'; document.querySelector('.mhs-del-btn[data-id=\\'${e.id}\\']')?.click(); }"
    ...
    </div>`,t.style.display=`flex`;let m=n.querySelector(`#mhsAccSection`);try{let t=await fetch(`${Xt}/account/${e.id}`);if(t.ok){let n=await t.json(),r=n.plain_password,i=`mhsPwdToggle_`+e.id,a=n.is_validated?`<span style="color:hsl(145 55% 40%);font-weight:600;">✅ Tervalidasi</span> <span style="font-size:0.72rem;color:var(--text-muted);">oleh ${n.validated_by||`BAP`} · ${s(n.validated_at)}</span>`:`<span style="color:hsl(38 75% 45%);font-weight:600;">⏳ Belum Divalidasi</span>`;m.innerHTML=`
        <div style="background:linear-gradient(135deg,hsl(215 70% 96%),hsl(250 60% 97%));border-radius:12px;padding:14px 16px;border:1px solid hsl(215 50% 88%);">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">
            <div style="width:28px;height:28px;background:linear-gradient(135deg,hsl(215 65% 50%),hsl(250 65% 58%));border-radius:8px;display:flex;align-items:center;justify-content:center;">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
        ...
        </div>`}catch{m.innerHTML=`<div style="font-size:0.75rem;color:var(--text-muted);padding:8px;">Gagal memuat info akun.</div>`}}async function an(e){let t=document.getElementById(`mhsModal`),n=document.getElementById(`mhsModalBody`),r=document.getElementById(`mhsModalTitle`);if(!t||!n)return;r&&(r.textContent=`Edit Data Mahasiswa`);let i=[`Administrasi Negara`,`Administrasi Niaga`],a=e=>e||``;n.innerHTML=`
    ...
    </form>`,t.style.display=`flex`;let o=n.querySelector(`#editAccSection`),s=null;try{let t=await fetch(`${Xt}/account/${e.id}`);if(t.ok){let n=await t.json();s=n.id;let r=`editPwd_`+e.id;o.innerHTML=`
        <h5 class="off-section-title">🔐 Info Akun Mahasiswa</h5>
        <div style="background:linear-gradient(135deg,hsl(215 70% 96%),hsl(250 60% 97%));border-radius:12px;padding:14px 16px;border:1px solid hsl(215 50% 88%);">
          <div class="off-row">
            <div class="form-group">
...
Data yang dihapus tidak dapat dikembalikan.`))return;let a=l.findIndex(e=>e.id===r);a>=0&&(l.splice(a,1),alert(`✅ Dosen berhasil dihapus`),e.innerHTML=dn()+t,fn(e,t))})})}var pn={mahasiswa:Me,dosen:He,kaprodi:it,bap:at};function mn(e){let t=tr();if(!t){window.location.hash=`#/login`;return}let n=pn[t.role]||Me;if(e.innerHTML=`
    ...
    <main class="portal-main">
      ...
      <div class="portal-welcome">
        <div class="portal-welcome-text">
          <h2>${hn()}, ${t.nama.split(` `)[0]}</h2>
          <p>${gn()} &nbsp;•&nbsp; <span class="portal-clock-icon">${G.clock}</span> <span id="portalClock">${_n()}</span></p>
  ...
  </div>`,document.getElementById(`portalLogoutBtn`)?.addEventListener(`click`,()=>nr()),t.role===`mahasiswa`&&t.nim&&fetch(`/api/transkip/${encodeURIComponent(t.nim)}`).then(e=>e.ok?e.json():null).then(e=>{if(!e)return;let t=e.ipk?Number(e.ipk).toFixed(2):`—`,n=e.totalSks??e.total_sks??`—`,r=document.getElementById(`portalStat0`),i=document.getElementById(`portalStat1`);r&&(r.textContent=t),i&&(i.textContent=n)}).catch(()=>{});let a=document.getElementById(`portalMobileToggle`),o=document.getElementById(`portalSidebar`),s=document.getElementById(`portalMobileOverlay`);a?.addEventListener(`click`,()=>{o?.classList.toggle(`open`),s?.classList.toggle(`open`)}),s?.addEventListener(`click`,()=>{o?.classList.remove(`open`),s?.classList.remove(`open`)});let c=document.getElementById(`portalClock`);if(c){let e=setInterval(()=>{if(!document.getElementById(`portalClock`)){clearInterval(e);return}c.textContent=_n()},1e3)}document.querySelectorAll(`[data-coming-soon]`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault(),xn(e.getAttribute(`data-coming-soon`))})})}function xn(e){document.querySelector(`.coming-soon-toast`)?.remove();let t=document.createElement(`div`);t.className=`coming-soon-toast`,t.innerHTML=`
  ...
  <aside class="dash-sidebar elearning-sidebar" id="dashSidebar" role="navigation" aria-label="Menu navigasi E-Learning">
    ...
    <div class="sidebar-user" role="status" aria-label="Informasi pengguna aktif">
      ...
      <div>
        <div class="sidebar-user-name">${e.nama}</div>
        <div class="sidebar-user-role">${Cn[e.role]}${e.nim?` — `+e.nim:``}</div>
      </div>
    </div>
    ${t.map(e=>`
  ...
  <header class="dash-topbar" role="banner" aria-label="Header E-Learning">
    ...
    <div class="el-welcome-banner">
      ...
      <div class="el-welcome-text">
        ...
        <h2>${wn()}, ${(e.nama||e.name||`Mahasiswa`).split(` `)[0]}! 👋</h2>
    ...
    </div>`}var On=[];function kn(e){let t=On.length>0?On:b,n=t.slice(0,4);return`
    <div class="stat-grid">
      <div class="stat-box">
    ...
    </div>`}function Fn(e,t){let n=Y[e.judul]||Nn(e.judul,e.soal),r=n.length;return`
    ...
    </div>`}function In(e,t,n,r,i){let a=Math.round(t/n*100),o=a>=60;return`
  ...
  <div class="dash elearning" role="application" aria-label="E-Learning - Sistem Pembelajaran Online">
    ...
    <div class="pmb-form-container">
      ...
      <form id="pmbForm" class="pmb-form">
        ...
        <div class="pmb-section">
          ...
          <div class="pmb-row">
            ${Q(`Pilih Program Studi *`,`prodi_pilihan`,`text`,!0,Wn)}
          </div>
        </div>
    ...
    <div class="pmb-form-container">
      ...
      <p class="pmb-form-sub">Biaya pendaftaran: <strong>Rp ${Un.toLocaleString(`id-ID`)}</strong></p>
    ...
    <div class="pmb-form-container">
      ...
      <div class="pmb-info-cards">
        ...
        <div class="pmb-info-card">
          ...
          <table class="pmb-info-table">
            <tr><td>Biaya Formulir</td><td><strong>Rp ${Un.toLocaleString(`id-ID`)}</strong></td></tr>
        ...
        <div class="pmb-info-card">
          <h3>🎓 Program Studi</h3>
          <ul class="pmb-req-list">
            ${Wn.map(e=>`<li>${e}</li>`).join(``)}
          </ul>
        </div>
      </div>
    </div>`}async function $n(e){let t={total_pendaftar:0,total_proses:0,total_diterima:0,total_ditolak:0};try{let[e,n]=await Promise.all([fetch(`${X}/stats`),fetch(`${X}/settings/biaya`)]);e.ok&&(t=await e.json()),n.ok&&(Un=(await n.json()).biaya_pendaftaran||35e4)}catch(e){console.warn(`Init fetch failed:`,e)}function n(e){let t=document.getElementById(`pmbMainArea`);if(t){switch(e){case`info`:t.innerHTML=Qn();break;case`daftar`:t.innerHTML=Yn();break;case`status`:t.innerHTML=Xn();break;case`bayar`:t.innerHTML=Zn();break}document.querySelectorAll(`.pmb-nav-link`).forEach(t=>{t.classList.toggle(`active`,t.dataset.pmbPage===e)}),r(e)}}e.innerHTML=`
    <div class="pmb-page">
      <!-- Header -->
        ...
        <span>${Z.checkCircle} ISO 40500 — Accessibility</span>
          ...
          <div class="pmb-payment-card">
            ...
            <div class="pmb-pay-amount">
              ...
              <strong>Rp ${Un.toLocaleString(`id-ID`)}</strong>
            </div>
            ...
            `}
                            ...
                            </div>`,fetch(`${X}/stats`).then(e=>e.json()).then(e=>{t=e;let n=document.querySelector(`.pmb-stats`);n&&(n.outerHTML=qn(t))}).catch(()=>{})},onPending:function(e){r.innerHTML=`
                      ...
                      </div>`;try{let e=await fetch(`${X}/stats`);e.ok&&(t=await e.json(),document.querySelector(`.pmb-stats`).outerHTML=qn(t))}catch{}}else alert(`❌ `+(o.error||`Gagal memproses pembayaran`)),e.disabled=!1,e.querySelector(`.pmb-pay-option-title`).textContent=n===`online`?`Bayar Online`:`Bayar Cash`}catch{alert(`❌ Gagal menghubungi server`),e.disabled=!1}})})}catch{r.innerHTML=`<div class="pmb-error">❌ Gagal menghubungi server</div>`}}e.addEventListener(`click`,r),n.addEventListener(`keydown`,e=>{e.key===`Enter`&&r()})}}var $={user:null,role:null,theme:localStorage.getItem(`theme`)||`light`};$.theme===`dark`&&document.documentElement.setAttribute(`data-theme`,`dark`);function er(e){$.user=e,$.role=e?.role||null,sessionStorage.setItem(`user`,JSON.stringify(e))}function tr(){if(!$.user){let e=sessionStorage.getItem(`user`);e&&($.user=JSON.parse(e),$.role=$.user?.role||null)}return $.user}function nr(){$.user=null,$.role=null,sessionStorage.removeItem(`user`),i(`#/login`)}r(`/`,e=>ie(e),`Beranda`),r(`/login`,e=>ue(e),`Login`),r(`/portal`,e=>bn(e),`Portal`),r(`/dashboard`,e=>mn(e),`SIAKAD`),r(`/elearning`,e=>Hn(e),`E-Learning`),r(`/pmb`,e=>$n(e),`PMB`),document.addEventListener(`DOMContentLoaded`,()=>{a(`app`)});