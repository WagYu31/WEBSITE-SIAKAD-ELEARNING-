(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e={},t={},n=null;function r(n,r,i=``){e[n]=r,t[n]=i}function i(e){window.location.hash=e}function a(e){n=document.getElementById(e),window.addEventListener(`hashchange`,o),window.addEventListener(`load`,o),window.location.hash?o():window.location.hash=`#/`}async function o(){let r=window.location.hash.slice(1)||`/`,i=e[r];if(!i){for(let[t,n]of Object.entries(e))if(t.includes(`:`)){let e=RegExp(`^`+t.replace(/:[^/]+/g,`([^/]+)`)+`$`);if(r.match(e)){i=n;break}}}if(i&&n){n.style.opacity=`0`,n.style.transform=`translateY(10px)`,await new Promise(e=>setTimeout(e,150));try{await i(n)}catch(e){console.error(`Route error:`,e),n.innerHTML=`
  ...
  <aside class="dash-sidebar" id="dashSidebar" role="navigation" aria-label="Menu navigasi SIAKAD">
    ...
    <div class="sidebar-user" role="status" aria-label="Informasi pengguna aktif">
      ...
      <div>
        ...
        <div class="sidebar-user-role">${be[e.role]}${e.nim?` — `+e.nim:``}</div>
      </div>
    </div>
    ${t.map(e=>`
  ...
  <header class="dash-topbar" role="banner" aria-label="Header SIAKAD">
    <div class="dash-topbar-left">
      ...
      <div>
        <h1>${ye()}, ${e.nama.split(` `)[0]}!</h1>
        <p>Semester Genap ${new Date().getFullYear()} — ${be[e.role]}</p>
      </div>
    </div>
  ...
  </header>`}function Ce(e){p.reduce((e,t)=>e+t.sks,0);let t=[`Minggu`,`Senin`,`Selasa`,`Rabu`,`Kamis`,`Jumat`,`Sabtu`][new Date().getDay()],n=T.mataKuliah.filter(e=>e.waktu.startsWith(t)).map(e=>({jam:e.waktu.split(`, `)[1]||e.waktu,mk:e.nama,ruang:e.ruang||`TBA`,dosen:e.dosen})),r=x.filter(e=>e.status===`Belum`);return`
    <div class="stat-grid">
      <div class="stat-box">
    ...
    </div>`}function Te(e){let t=T.mataKuliah.reduce((e,t)=>e+t.sks,0);return we(e)+`
    ...
    <div class="dash-card" style="margin-top:20px;">
      ...
      </div>`}).join(``)}let o=Object.values(n).reduce((e,t)=>e+t.length,0);return we(e)+`
    <div class="dash-card">
    ...
    </div>`}function L(e){return`<div class="dash-card" style="margin-bottom:20px;overflow:hidden;">
  ...
  </div>`}function Pe(e){let t=[`Senin`,`Selasa`,`Rabu`,`Kamis`,`Jumat`],n=k.map((e,t)=>({hari:e.hari,jam:e.jam,kode:e.kode,nama:e.nama,kelas:e.kelas,ruang:e.ruang,jmlMhs:e.mahasiswa.length,idx:t}));return`${L(e)}
    <div class="dash-card">
    ...
    </div>`).join(``)}`}function Re(e){function t(e){return e>=85?`A`:e>=80?`A-`:e>=75?`B+`:e>=70?`B`:e>=65?`B-`:e>=60?`C+`:e>=55?`C`:e>=45?`D`:`E`}return`${L(e)}
    ...
    `),i.document.close(),setTimeout(()=>i.print(),500)}}function Ke(e){let t=(A.reduce((e,t)=>e+t.ipk,0)/A.length).toFixed(2);return`${L(e)}
    <div class="dash-card" style="margin-bottom:16px;">
    ...
    </div>`}function Ye(){try{let e=localStorage.getItem(`siakad_kelas_list`);if(e)return JSON.parse(e)}catch{}return[`RA-101`,`RA-201`,`RA-202`,`RA-203`,`RA-204`,`RA-205`,`RN-101`,`RN-102`,`RN-103`,`RN-104`,`RN-105`,`LAB-K1`]}function Xe(e){localStorage.setItem(`siakad_kelas_list`,JSON.stringify(e))}function Ze(){let e=new Date().getFullYear();Ye();let t=[];[`niaga`,`negara`].forEach(e=>{let n=w[e];n&&n.semester.forEach(r=>{r.mk.forEach(i=>{t.push({...i,prodi:e,prodiNama:n.nama,semester:r.no})})})}),l.filter(e=>e.status===`Aktif`).map(e=>e.nama),ge();let n={Offline:`hsl(210 55% 50%)`,Online:`hsl(150 55% 45%)`,Hybrid:`hsl(275 55% 55%)`},r={Offline:`hsl(210 50% 94%)`,Online:`hsl(150 50% 94%)`,Hybrid:`hsl(275 50% 94%)`},i={niaga:`hsl(35 75% 50%)`,negara:`hsl(145 55% 45%)`},a={niaga:`hsl(35 70% 94%)`,negara:`hsl(145 50% 93%)`},o=JADWAL_DUMMY.filter(e=>e.tipeKelas===`Online`).length,s=JADWAL_DUMMY.filter(e=>e.tipeKelas===`Offline`).length,c=JADWAL_DUMMY.filter(e=>e.tipeKelas===`Hybrid`).length,u=JADWAL_DUMMY.filter(e=>e.prodi===`niaga`).length,d=JADWAL_DUMMY.filter(e=>e.prodi===`negara`).length;return`
    <div class="dash-card" style="margin-bottom:16px;overflow:hidden;">
    ...
    </div>`}async function vt(){document.querySelectorAll(`.pmb-mgmt-btn`).forEach(e=>{e.addEventListener(`click`,()=>{document.querySelectorAll(`.pmb-mgmt-btn`).forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),e.dataset.tab===`list`?B():e.dataset.tab===`add`&&Mt()})}),document.getElementById(`closeDetailModal`)?.addEventListener(`click`,()=>{document.getElementById(`pmbDetailModal`)?.classList.remove(`active`)}),document.getElementById(`pmbDetailModal`)?.addEventListener(`click`,e=>{e.target.id===`pmbDetailModal`&&e.target.classList.remove(`active`)}),B()}var z=[];async function B(){let e=document.getElementById(`pmbMgmtContent`);if(e){e.innerHTML=`<div style="text-align:center;padding:24px;">
  ...
  </div>`;try{let[e,t]=await Promise.all([fetch(`${R}/registrations`),fetch(`${R}/stats`)]),n=await e.json(),r=await t.json();z=n.data||[],bt(r,z)}catch{e.innerHTML=`<div style="text-align:center;padding:24px;">
    ...
    </div>`}}}var V=1,yt=20,H={key:`created_at`,dir:`desc`},U=new Set;function bt(e,t){let n=document.getElementById(`pmbMgmtContent`);if(!n)return;n.innerHTML=`
    <!-- Stat Cards -->
    ...
    </div>`;let r=()=>{V=1,xt()};document.getElementById(`pmbSearch`)?.addEventListener(`input`,r),document.getElementById(`pmbFilterStatus`)?.addEventListener(`change`,r),document.getElementById(`pmbFilterProdi`)?.addEventListener(`change`,r),document.getElementById(`pmbDateFrom`)?.addEventListener(`change`,r),document.getElementById(`pmbDateTo`)?.addEventListener(`change`,r),document.getElementById(`pmbClearDate`)?.addEventListener(`click`,()=>{document.getElementById(`pmbDateFrom`).value=``,document.getElementById(`pmbDateTo`).value=``,r()}),document.getElementById(`pmbExportBtn`)?.addEventListener(`click`,()=>At(z)),document.getElementById(`pmbBulkValidate`)?.addEventListener(`click`,Tt),document.getElementById(`pmbBulkAccount`)?.addEventListener(`click`,Et),document.getElementById(`pmbBulkDelete`)?.addEventListener(`click`,Dt),document.getElementById(`pmbBulkCancel`)?.addEventListener(`click`,()=>{U.clear(),wt(),xt()}),xt()}function xt(){let e=(document.getElementById(`pmbSearch`)?.value||``).toLowerCase(),t=document.getElementById(`pmbFilterStatus`)?.value||``,n=document.getElementById(`pmbFilterProdi`)?.value||``,r=document.getElementById(`pmbDateFrom`)?.value||``,i=document.getElementById(`pmbDateTo`)?.value||``,a=z.filter(a=>{let o=!e||(a.nama||``).toLowerCase().includes(e)||(a.nik||``).includes(e)||(a.no_pendaftaran||``).toLowerCase().includes(e),s=!t||a.status===t,c=!n||a.prodi_pilihan===n,l=!0;if(r||i){let e=a.created_at?a.created_at.slice(0,10):``;r&&e<r&&(l=!1),i&&e>i&&(l=!1)}return o&&s&&c&&l}),{key:o,dir:s}=H;a.sort((e,t)=>{let n=e[o]||``,r=t[o]||``;return typeof n==`string`&&(n=n.toLowerCase()),typeof r==`string`&&(r=r.toLowerCase()),n<r?s===`asc`?-1:1:n>r?s===`asc`?1:-1:0});let c=a.length,l=Math.max(1,Math.ceil(c/yt));V>l&&(V=l);let u=(V-1)*yt,d=a.slice(u,u+yt);document.getElementById(`pmbTableWrapper`).innerHTML=Ct(d),document.getElementById(`pmbResultCount`).textContent=c>0?`Menampilkan ${u+1}–${Math.min(u+yt,c)} dari ${c} pendaftar`:`Tidak ada data`,St(l),Ot()}function St(e){let t=document.getElementById(`pmbPagination`);if(!t||e<=1){t&&(t.innerHTML=``);return}let n=e=>`style="padding:4px 10px;border-radius:6px;border:1px solid ${e?`var(--primary-500)`:`var(--gray-200)`};background:${e?`var(--primary-500)`:`#fff`};color:${e?`#fff`:`var(--text-primary)`};cursor:pointer;font-size:0.75rem;font-weight:600;"`,r=`<button ${n(!1)} ${V<=1?`disabled`:``} data-pg="${V-1}">‹</button>`,i=Math.max(1,V-2),a=Math.min(e,i+4);a-i<4&&(i=Math.max(1,a-4));for(let e=i;e<=a;e++)r+=`<button ${n(e===V)} data-pg="${e}">${e}</button>`;r+=`<button ${n(!1)} ${V>=e?`disabled`:``} data-pg="${V+1}">›</button>`,t.innerHTML=r,t.querySelectorAll(`button[data-pg]`).forEach(t=>t.addEventListener(`click`,()=>{let n=parseInt(t.dataset.pg);n>=1&&n<=e&&(V=n,xt())}))}function Ct(e){if(e.length===0)return`<div style="text-align:center;padding:32px;">
    ...
    </table>`}function wt(){let e=document.getElementById(`pmbBulkBar`),t=document.getElementById(`pmbBulkCount`);e&&(U.size>0?(e.style.display=`flex`,t&&(t.textContent=`${U.size} pendaftar dipilih`)):e.style.display=`none`)}async function Tt(){if(!confirm(`Validasi ${U.size} pendaftar sekaligus?`))return;let e=0,t=0;for(let n of U)try{(await fetch(`${R}/account/${n}/validate`,{method:`PUT`})).ok?e++:t++}catch{t++}alert(`✅ Berhasil: ${e}\n❌ Gagal: ${t}`),U.clear(),B()}async function Et(){if(!confirm(`Buat akun untuk ${U.size} pendaftar sekaligus?`))return;let e=0,t=0;for(let n of U)try{(await fetch(`${R}/account/create`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({registration_id:n})})).ok?e++:t++}catch{t++}alert(`✅ Berhasil: ${e}\n❌ Gagal: ${t}`),U.clear(),B()}async function Dt(){if(!confirm(`⚠️ HAPUS ${U.size} pendaftar?\n\nAksi ini tidak bisa dibatalkan!`))return;let e=0,t=0;for(let n of U)try{(await fetch(`${R}/registration/${n}`,{method:`DELETE`})).ok?e++:t++}catch{t++}alert(`✅ Terhapus: ${e}\n❌ Gagal: ${t}`),U.clear(),B()}function Ot(){document.querySelectorAll(`.mgmt-action-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation(),jt(e.dataset.action,e.dataset)})}),document.querySelectorAll(`.pmb-tr`).forEach(e=>{e.addEventListener(`click`,()=>{let t=parseInt(e.dataset.id),n=z.find(e=>e.id===t);n&&kt(n)})}),document.querySelectorAll(`.sortable-th`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.sort;H.key===t?H.dir=H.dir===`asc`?`desc`:`asc`:H={key:t,dir:`asc`},xt()})}),document.getElementById(`pmbSelectAll`)?.addEventListener(`change`,e=>{document.querySelectorAll(`.pmb-chk`).forEach(t=>{let n=parseInt(t.dataset.id);e.target.checked?U.add(n):U.delete(n),t.checked=e.target.checked}),wt()}),document.querySelectorAll(`.pmb-chk`).forEach(e=>{e.addEventListener(`change`,()=>{let t=parseInt(e.dataset.id);e.checked?U.add(t):U.delete(t),wt()})})}function kt(e){let t=document.getElementById(`pmbDetailModal`),n=document.getElementById(`pmbDetailContent`);if(!t||!n)return;let r={diterima:`badge-success`,ditolak:`badge-danger`,proses:`badge-primary`,menunggu:`badge-warning`},i=e=>e||`<span style="color:var(--danger-400);font-style:italic;">— kosong</span>`,a=e=>e?`✅`:`❌`,o=e=>e?new Date(e).toLocaleDateString(`id-ID`,{day:`numeric`,month:`long`,year:`numeric`}):`-`,s=[`nik`,`nama`,`email`,`telepon_1`,`prodi_pilihan`,`asal_sekolah`,`alamat`,`tempat_lahir`,`tanggal_lahir`,`gender`],c=s.filter(t=>e[t]&&String(e[t]).trim()).length,l=Math.round(c/s.length*100),u=l===100?`hsl(145 60% 45%)`:l>=70?`hsl(38 90% 50%)`:`hsl(0 70% 55%)`;n.innerHTML=`
...
`),n=new Blob([t],{type:`text/csv`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=`PMB_Data_${new Date().toISOString().slice(0,10)}.csv`,i.click(),URL.revokeObjectURL(r)}async function jt(e,t){try{let n,r;switch(e){case`create-account`:if(n=await fetch(`${R}/account/create`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({registration_id:parseInt(t.id)})}),r=await n.json(),n.ok){let e=r.password;alert(e?`✅ Akun dibuat!\n\nNIM: ${r.nim}\nPassword: ${e}\nEmail: ${r.email}`:`ℹ️ Akun sudah dibuat sebelumnya.\n\nNIM: ${r.nim}\nEmail: ${r.email}\n\nPassword hanya ditampilkan saat pertama kali dibuat.`)}else alert(`❌ `+(r.error||`Gagal membuat akun`));break;case`validate`:let e=await fetch(`${R}/account/${t.id}`);if(!e.ok){alert(`❌ Akun belum dibuat. Buat akun terlebih dahulu.`);return}if((await e.json()).is_validated){alert(`ℹ️ Akun sudah divalidasi sebelumnya.`);return}let i=await fetch(`${R}/account/${t.id}/validate`,{method:`PUT`});r=await i.json(),i.ok?(alert(`✅ `+r.message),B()):alert(`❌ `+(r.error||`Gagal validasi`));break;case`confirm-pay`:let a=await fetch(`${R}/payment`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({registration_id:parseInt(t.id),metode_bayar:`cash`,jumlah:35e4})});if(a.ok){let e=await a.json();(await fetch(`${R}/payment/${e.id}/confirm`,{method:`PUT`})).ok&&(alert(`✅ Pembayaran cash dikonfirmasi!`),B())}else{let e=await a.json();e.error&&e.error.includes(`sudah`)?alert(`ℹ️ Pembayaran sudah tercatat.`):alert(`❌ `+(e.error||`Gagal`))}break;case`edit`:Nt(parseInt(t.id));return;case`delete`:let o=z.find(e=>e.id===parseInt(t.id));if(!o||!confirm(`⚠️ Hapus data pendaftaran?\n\n${o.nama} (${o.no_pendaftaran})\n\nSemua data terkait (akun, pembayaran) juga akan dihapus.`))return;n=await fetch(`${R}/registration/${t.id}`,{method:`DELETE`}),r=await n.json(),n.ok?(alert(`✅ `+r.message),B()):alert(`❌ `+(r.error||`Gagal menghapus`));break}}catch(e){alert(`❌ Error: `+e.message)}}function Mt(){let e=document.getElementById(`pmbMgmtContent`);e&&(e.innerHTML=`
    <form id="offlineRegForm" style="max-width:640px;">
      ...
      <button type="submit" class="btn btn-primary" id="offlineSubmitBtn" style="width:100%;padding:14px;">
        ✨ Daftarkan & Buat Akun
          ...
          </div>`}else alert(`❌ `+(r.error||`Gagal`)),n.disabled=!1,n.textContent=`📝 Daftarkan & Buat Akun`}catch(e){alert(`❌ `+e.message),n.disabled=!1,n.textContent=`📝 Daftarkan & Buat Akun`}}))}function Nt(e){let t=z.find(t=>t.id===e);if(!t)return;let n=document.getElementById(`pmbDetailModal`),r=document.getElementById(`pmbDetailContent`);if(!n||!r)return;let i=[`Administrasi Negara`,`Administrasi Niaga`],a=e=>e||``;r.innerHTML=`
    ...
    </div>`}function It(){Lt(),document.getElementById(`mhsSearch`)?.addEventListener(`input`,zt),document.getElementById(`mhsFilterProdi`)?.addEventListener(`change`,zt),document.getElementById(`mhsFilterStatus`)?.addEventListener(`change`,zt)}async function Lt(){try{let e=await fetch(`${Pt}/registrations`);if(!e.ok)throw Error(`Gagal memuat data`);let t=await e.json();W=(Array.isArray(t)?t:t.data||[]).filter(e=>e.status===`diterima`).map((e,t)=>({...e,nim:e.nim||`20260${String(t+1).padStart(4,`0`)}`,angkatan:e.angkatan||(e.created_at?new Date(e.created_at).getFullYear():2026),semester:e.semester||1,status_mhs:e.status_mhs||`aktif`})),Rt(),Bt(W)}catch(e){document.getElementById(`mhsTableContainer`).innerHTML=`
      <div style="text-align:center;padding:32px;">
        <p style="color:var(--text-muted);font-size:var(--text-sm);">⚠️ ${e.message}</p>
      </div>`}}function Rt(){let e=W.length,t=W.filter(e=>e.status_mhs===`aktif`).length,n=W.filter(e=>e.status_mhs===`cuti`).length,r=W.filter(e=>e.status_mhs===`lulus`).length,i=e=>document.getElementById(e);i(`mhsTotal`)&&(i(`mhsTotal`).textContent=e),i(`mhsAktif`)&&(i(`mhsAktif`).textContent=t),i(`mhsCuti`)&&(i(`mhsCuti`).textContent=n),i(`mhsLulus`)&&(i(`mhsLulus`).textContent=r)}function zt(){let e=(document.getElementById(`mhsSearch`)?.value||``).toLowerCase(),t=document.getElementById(`mhsFilterProdi`)?.value||``,n=document.getElementById(`mhsFilterStatus`)?.value||``;Bt(W.filter(r=>{let i=!e||(r.nama||``).toLowerCase().includes(e)||(r.nim||``).toLowerCase().includes(e)||(r.nik||``).includes(e),a=!t||r.prodi_pilihan===t,o=!n||r.status_mhs===n;return i&&a&&o}))}function Bt(e){let t=document.getElementById(`mhsTableContainer`),n=document.getElementById(`mhsCount`);if(t){if(e.length===0){t.innerHTML=`<div style="text-align:center;padding:32px;">
      <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="var(--gray-300)" stroke-width="1.5" style="margin:0 auto 12px;display:block;"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>
    ...
    </table>`,n&&(n.textContent=`Menampilkan ${e.length} mahasiswa`),t.querySelectorAll(`.mhs-tr`).forEach(e=>{e.addEventListener(`click`,()=>{let t=parseInt(e.dataset.id),n=W.find(e=>e.id===t);n&&Vt(n)})}),t.querySelectorAll(`.mhs-edit-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=parseInt(e.dataset.id),n=W.find(e=>e.id===t);n&&Ht(n)})}),t.querySelectorAll(`.mhs-del-btn`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=parseInt(e.dataset.id),n=W.find(e=>e.id===t);if(n&&confirm(`⚠️ Hapus data mahasiswa ${n.nama}?\n\nAkun dan data terkait akan dihapus.`))try{let e=await fetch(`${R}/registration/${t}`,{method:`DELETE`});if(e.ok)alert(`✅ Data mahasiswa berhasil dihapus`),Lt();else{let t=await e.json();alert(`❌ `+(t.error||`Gagal menghapus`))}}catch(e){alert(`❌ `+e.message)}})})}}function Vt(e){let t=document.getElementById(`mhsModal`),n=document.getElementById(`mhsModalBody`),r=document.getElementById(`mhsModalTitle`);if(!t||!n)return;r&&(r.textContent=`Profil Mahasiswa`);let i=(e,t)=>`
  ...
  `,t.classList.add(`active`)}function Ht(e){let t=document.getElementById(`mhsModal`),n=document.getElementById(`mhsModalBody`),r=document.getElementById(`mhsModalTitle`);if(!t||!n)return;r&&(r.textContent=`Edit Data Mahasiswa`);let i=[`Administrasi Negara`,`Administrasi Niaga`],a=e=>e||``;n.innerHTML=`
...
Data yang dihapus tidak dapat dikembalikan.`))return;let a=l.findIndex(e=>e.id===r);a>=0&&(l.splice(a,1),alert(`✅ Dosen berhasil dihapus`),e.innerHTML=Jt()+t,Yt(e,t))})})}var Xt={mahasiswa:Ce,dosen:Ne,kaprodi:qe,bap:Je};function Zt(e){let t=In();if(!t){window.location.hash=`#/login`;return}let n=Xt[t.role]||Ce;e.innerHTML=`
    ...
    <main class="portal-main">
      ...
      <div class="portal-welcome">
        <div class="portal-welcome-text">
          <h2>${Qt()}, ${t.nama.split(` `)[0]}</h2>
          <p>${$t()} &nbsp;•&nbsp; <span class="portal-clock-icon">${G.clock}</span> <span id="portalClock">${en()}</span></p>
  ...
  <aside class="dash-sidebar elearning-sidebar" id="dashSidebar" role="navigation" aria-label="Menu navigasi E-Learning">
    ...
    <div class="sidebar-user" role="status" aria-label="Informasi pengguna aktif">
      ...
      <div>
        <div class="sidebar-user-name">${e.nama}</div>
        <div class="sidebar-user-role">${on[e.role]}${e.nim?` — `+e.nim:``}</div>
      </div>
    </div>
    ${t.map(e=>`
  ...
  <header class="dash-topbar" role="banner" aria-label="Header E-Learning">
    ...
    </div>`}function _n(e,t){let n=Y[e.judul]||hn(e.judul,e.soal),r=n.length;return`
    ...
    </div>`}function vn(e,t,n,r,i){let a=Math.round(t/n*100),o=a>=60;return`
    ...
    <div class="el-modal-overlay" id="nilaiSayaModal" style="display:none">
    ...
    </div>`}var Sn={home:null,kelas:xn};function Cn(e,t,n){if(n&&n.activeForum)return yn(n.activeForum.forum,n.activeForum.classId,t.nama||`User`,t.role);if(n&&n.quizIntro)return gn(n.quizIntro);if(n&&n.quizResult){let e=n.quizResult;return vn(e.quiz,e.score,e.total,e.answers,e.questions)}if(n&&n.activeQuiz)return _n(n.activeQuiz,n.kelas.id);if(n&&n.kelas)return bn(n.kelas,n.tab,t.role,t.nim);let r=pn[t.role]||un,i=Sn[e];return i?i(t):r(t)}function wn(e){let t=In();if(!t){window.location.hash=`#/login`;return}let n=`home`,r=null,i={},a=null;function o(){let e=document.getElementById(`dashMain`);e&&(e.innerHTML=Cn(n,t,r)+`
      <footer class="dash-iso-footer" role="contentinfo" aria-label="Sertifikasi ISO">
      ...
      `).join(``),e.querySelectorAll(`.el-qe-delete`).forEach(e=>{e.addEventListener(`click`,()=>{let t=parseInt(e.dataset.qi),n=Y[w.judul];n&&(n.splice(t,1),w.soal=n.length,E())})})}document.querySelectorAll(`.el-edit-quiz`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=parseInt(e.dataset.classId),r=parseInt(e.dataset.quizIdx),i=q[n];!i||!i.quiz[r]||(w=i.quiz[r],Y[w.judul]||(Y[w.judul]=[]),document.getElementById(`quizEditorTitle`).textContent=`📝 `+w.judul,E(),C.style.display=`flex`)})}),document.getElementById(`closeQuizEditor`)?.addEventListener(`click`,()=>{T(),o()}),C?.addEventListener(`click`,e=>{e.target===C&&(T(),o())}),document.getElementById(`addQuizQuestion`)?.addEventListener(`click`,()=>{if(!w)return;let e=document.getElementById(`qeSoal`)?.value.trim(),t=document.getElementById(`qeA`)?.value.trim(),n=document.getElementById(`qeB`)?.value.trim(),r=document.getElementById(`qeC`)?.value.trim(),i=document.getElementById(`qeD`)?.value.trim(),a=document.getElementById(`qeJawaban`)?.value;if(!e||!t||!n||!r||!i){alert(`Mohon isi pertanyaan dan semua pilihan A-D.`);return}Y[w.judul]||(Y[w.judul]=[]),Y[w.judul].push({soal:e,jawaban:a,opsi:[{key:`A`,text:t},{key:`B`,text:n},{key:`C`,text:r},{key:`D`,text:i}]}),w.soal=Y[w.judul].length,document.getElementById(`qeSoal`).value=``,document.getElementById(`qeA`).value=``,document.getElementById(`qeB`).value=``,document.getElementById(`qeC`).value=``,document.getElementById(`qeD`).value=``,E()});let D=document.getElementById(`filePreviewModal`),O=100;function k(e,t){if(!D)return;document.getElementById(`previewFileName`).textContent=`📄 `+e,document.getElementById(`previewStudentName`).textContent=`oleh `+t,O=100,document.getElementById(`previewZoomLevel`).textContent=`100%`;let n=e.split(`.`).pop().toLowerCase()===`pdf`,r=e.replace(/\.[^.]+$/,``).replace(/_/g,` `),i=`
        ...
        </div>`}).join(``),e.querySelectorAll(`.el-file-preview-btn`).forEach(e=>{e.addEventListener(`click`,()=>{k(e.dataset.file,e.dataset.student)})})}document.querySelectorAll(`.el-grade-tugas`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=parseInt(e.dataset.classId),r=parseInt(e.dataset.tugasIdx),i=q[n];!i||!i.tugas[r]||(F=i.tugas[r],J[F.judul]||(J[F.judul]=[]),document.getElementById(`gradingTitle`).textContent=`📊 `+F.judul,ne(),P.style.display=`flex`)})}),document.getElementById(`closeGrading`)?.addEventListener(`click`,te),P?.addEventListener(`click`,e=>{e.target===P&&te()}),document.getElementById(`saveGrades`)?.addEventListener(`click`,()=>{if(!F)return;let e=J[F.judul]||[];document.querySelectorAll(`.el-grade-input`).forEach(t=>{let n=parseInt(t.dataset.si);e[n]&&t.value!==``&&(e[n].nilai=parseInt(t.value))});let t=e.filter(e=>e.nilai!==null&&e.nilai!==void 0);t.length>0&&(F.nilai=Math.round(t.reduce((e,t)=>e+t.nilai,0)/t.length)),te(),o()})}e.innerHTML=`
  <a href="#dashMain" class="skip-to-content" aria-label="Langsung ke konten utama">Skip to content</a>
    ...
    <div class="pmb-form-container">
      ...
      <form id="pmbForm" class="pmb-form">
        ...
        <div class="pmb-section">
          ...
          <div class="pmb-row">
            ${Q(`Pilih Program Studi *`,`prodi_pilihan`,`text`,!0,Tn)}
          </div>
        </div>
    ...
    <div class="pmb-form-container">
      ...
      <div class="pmb-info-cards">
        ...
        <div class="pmb-info-card">
          <h3>🎓 Program Studi</h3>
          <ul class="pmb-req-list">
            ${Tn.map(e=>`<li>${e}</li>`).join(``)}
          </ul>
        </div>
      </div>
    </div>`}async function Pn(e){let t={total_pendaftar:0,total_proses:0,total_diterima:0,total_ditolak:0};try{let e=await fetch(`${X}/stats`);e.ok&&(t=await e.json())}catch(e){console.warn(`Stats fetch failed:`,e)}function n(e){let t=document.getElementById(`pmbMainArea`);if(t){switch(e){case`info`:t.innerHTML=Nn();break;case`daftar`:t.innerHTML=An();break;case`status`:t.innerHTML=jn();break;case`bayar`:t.innerHTML=Mn();break}document.querySelectorAll(`.pmb-nav-link`).forEach(t=>{t.classList.toggle(`active`,t.dataset.pmbPage===e)}),r(e)}}e.innerHTML=`
    <div class="pmb-page">
      <!-- Header -->
        ...
        <span>${Z.checkCircle} ISO 40500 — Accessibility</span>
          ...
          <div class="pmb-payment-card">
            ...
            `}
                      ...
                      </div>`;try{let e=await fetch(`${X}/stats`);e.ok&&(t=await e.json(),document.querySelector(`.pmb-stats`).outerHTML=On(t))}catch{}}else alert(`❌ `+(o.error||`Gagal memproses pembayaran`)),e.disabled=!1,e.querySelector(`.pmb-pay-option-title`).textContent=n===`online`?`Bayar Online`:`Bayar Cash`}catch{alert(`❌ Gagal menghubungi server`),e.disabled=!1}})})}catch{r.innerHTML=`<div class="pmb-error">❌ Gagal menghubungi server</div>`}}e.addEventListener(`click`,r),n.addEventListener(`keydown`,e=>{e.key===`Enter`&&r()})}}var $={user:null,role:null,theme:localStorage.getItem(`theme`)||`light`};$.theme===`dark`&&document.documentElement.setAttribute(`data-theme`,`dark`);function Fn(e){$.user=e,$.role=e?.role||null,sessionStorage.setItem(`user`,JSON.stringify(e))}function In(){if(!$.user){let e=sessionStorage.getItem(`user`);e&&($.user=JSON.parse(e),$.role=$.user?.role||null)}return $.user}function Ln(){$.user=null,$.role=null,sessionStorage.removeItem(`user`),i(`#/login`)}r(`/`,e=>ie(e),`Beranda`),r(`/login`,e=>ue(e),`Login`),r(`/portal`,e=>rn(e),`Portal`),r(`/dashboard`,e=>Zt(e),`SIAKAD`),r(`/elearning`,e=>wn(e),`E-Learning`),r(`/pmb`,e=>Pn(e),`PMB`),document.addEventListener(`DOMContentLoaded`,()=>{a(`app`)});