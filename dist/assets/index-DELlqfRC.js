(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e={},t={},n=null;function r(n,r,i=``){e[n]=r,t[n]=i}function i(e){window.location.hash=e}function a(e){n=document.getElementById(e),window.addEventListener(`hashchange`,o),window.addEventListener(`load`,o),window.location.hash?o():window.location.hash=`#/`}async function o(){let r=window.location.hash.slice(1)||`/`,i=e[r];if(!i){for(let[t,n]of Object.entries(e))if(t.includes(`:`)){let e=RegExp(`^`+t.replace(/:[^/]+/g,`([^/]+)`)+`$`);if(r.match(e)){i=n;break}}}if(i&&n){n.style.opacity=`0`,n.style.transform=`translateY(10px)`,await new Promise(e=>setTimeout(e,150));try{await i(n)}catch(e){console.error(`Route error:`,e),n.innerHTML=`
  ...
  <header class="dash-topbar" role="banner" aria-label="Header SIAKAD">
    ...
    </div>`}}}var V=1,pt=20,H={key:`created_at`,dir:`desc`},U=new Set;function mt(e,t){let n=document.getElementById(`pmbMgmtContent`);if(!n)return;n.innerHTML=`
    <!-- Stat Cards -->
    ...
    </div>`;let r=()=>{V=1,ht()};document.getElementById(`pmbSearch`)?.addEventListener(`input`,r),document.getElementById(`pmbFilterStatus`)?.addEventListener(`change`,r),document.getElementById(`pmbFilterProdi`)?.addEventListener(`change`,r),document.getElementById(`pmbDateFrom`)?.addEventListener(`change`,r),document.getElementById(`pmbDateTo`)?.addEventListener(`change`,r),document.getElementById(`pmbClearDate`)?.addEventListener(`click`,()=>{document.getElementById(`pmbDateFrom`).value=``,document.getElementById(`pmbDateTo`).value=``,r()}),document.getElementById(`pmbExportBtn`)?.addEventListener(`click`,()=>wt(z)),document.getElementById(`pmbBulkValidate`)?.addEventListener(`click`,yt),document.getElementById(`pmbBulkAccount`)?.addEventListener(`click`,bt),document.getElementById(`pmbBulkDelete`)?.addEventListener(`click`,xt),document.getElementById(`pmbBulkCancel`)?.addEventListener(`click`,()=>{U.clear(),vt(),ht()}),ht()}function ht(){let e=(document.getElementById(`pmbSearch`)?.value||``).toLowerCase(),t=document.getElementById(`pmbFilterStatus`)?.value||``,n=document.getElementById(`pmbFilterProdi`)?.value||``,r=document.getElementById(`pmbDateFrom`)?.value||``,i=document.getElementById(`pmbDateTo`)?.value||``,a=z.filter(a=>{let o=!e||(a.nama||``).toLowerCase().includes(e)||(a.nik||``).includes(e)||(a.no_pendaftaran||``).toLowerCase().includes(e),s=!t||a.status===t,c=!n||a.prodi_pilihan===n,l=!0;if(r||i){let e=a.created_at?a.created_at.slice(0,10):``;r&&e<r&&(l=!1),i&&e>i&&(l=!1)}return o&&s&&c&&l}),{key:o,dir:s}=H;a.sort((e,t)=>{let n=e[o]||``,r=t[o]||``;return typeof n==`string`&&(n=n.toLowerCase()),typeof r==`string`&&(r=r.toLowerCase()),n<r?s===`asc`?-1:1:n>r?s===`asc`?1:-1:0});let c=a.length,l=Math.max(1,Math.ceil(c/pt));V>l&&(V=l);let u=(V-1)*pt,d=a.slice(u,u+pt);document.getElementById(`pmbTableWrapper`).innerHTML=_t(d),document.getElementById(`pmbResultCount`).textContent=c>0?`Menampilkan ${u+1}–${Math.min(u+pt,c)} dari ${c} pendaftar`:`Tidak ada data`,gt(l),St()}function gt(e){let t=document.getElementById(`pmbPagination`);if(!t||e<=1){t&&(t.innerHTML=``);return}let n=e=>`style="padding:4px 10px;border-radius:6px;border:1px solid ${e?`var(--primary-500)`:`var(--gray-200)`};background:${e?`var(--primary-500)`:`#fff`};color:${e?`#fff`:`var(--text-primary)`};cursor:pointer;font-size:0.75rem;font-weight:600;"`,r=`<button ${n(!1)} ${V<=1?`disabled`:``} data-pg="${V-1}">‹</button>`,i=Math.max(1,V-2),a=Math.min(e,i+4);a-i<4&&(i=Math.max(1,a-4));for(let e=i;e<=a;e++)r+=`<button ${n(e===V)} data-pg="${e}">${e}</button>`;r+=`<button ${n(!1)} ${V>=e?`disabled`:``} data-pg="${V+1}">›</button>`,t.innerHTML=r,t.querySelectorAll(`button[data-pg]`).forEach(t=>t.addEventListener(`click`,()=>{let n=parseInt(t.dataset.pg);n>=1&&n<=e&&(V=n,ht())}))}function _t(e){if(e.length===0)return`<div style="text-align:center;padding:32px;">
    ...
    </table>`}function vt(){let e=document.getElementById(`pmbBulkBar`),t=document.getElementById(`pmbBulkCount`);e&&(U.size>0?(e.style.display=`flex`,t&&(t.textContent=`${U.size} pendaftar dipilih`)):e.style.display=`none`)}async function yt(){if(!confirm(`Validasi ${U.size} pendaftar sekaligus?`))return;let e=0,t=0;for(let n of U)try{(await fetch(`${R}/account/${n}/validate`,{method:`PUT`})).ok?e++:t++}catch{t++}alert(`✅ Berhasil: ${e}\n❌ Gagal: ${t}`),U.clear(),B()}async function bt(){if(!confirm(`Buat akun untuk ${U.size} pendaftar sekaligus?`))return;let e=0,t=0;for(let n of U)try{(await fetch(`${R}/account/create`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({registration_id:n})})).ok?e++:t++}catch{t++}alert(`✅ Berhasil: ${e}\n❌ Gagal: ${t}`),U.clear(),B()}async function xt(){if(!confirm(`⚠️ HAPUS ${U.size} pendaftar?\n\nAksi ini tidak bisa dibatalkan!`))return;let e=0,t=0;for(let n of U)try{(await fetch(`${R}/registration/${n}`,{method:`DELETE`})).ok?e++:t++}catch{t++}alert(`✅ Terhapus: ${e}\n❌ Gagal: ${t}`),U.clear(),B()}function St(){document.querySelectorAll(`.mgmt-action-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation(),Tt(e.dataset.action,e.dataset)})}),document.querySelectorAll(`.pmb-tr`).forEach(e=>{e.addEventListener(`click`,()=>{let t=parseInt(e.dataset.id),n=z.find(e=>e.id===t);n&&Ct(n)})}),document.querySelectorAll(`.sortable-th`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.sort;H.key===t?H.dir=H.dir===`asc`?`desc`:`asc`:H={key:t,dir:`asc`},ht()})}),document.getElementById(`pmbSelectAll`)?.addEventListener(`change`,e=>{document.querySelectorAll(`.pmb-chk`).forEach(t=>{let n=parseInt(t.dataset.id);e.target.checked?U.add(n):U.delete(n),t.checked=e.target.checked}),vt()}),document.querySelectorAll(`.pmb-chk`).forEach(e=>{e.addEventListener(`change`,()=>{let t=parseInt(e.dataset.id);e.checked?U.add(t):U.delete(t),vt()})})}function Ct(e){let t=document.getElementById(`pmbDetailModal`),n=document.getElementById(`pmbDetailContent`);if(!t||!n)return;let r={diterima:`badge-success`,ditolak:`badge-danger`,proses:`badge-primary`,menunggu:`badge-warning`},i=e=>e||`<span style="color:var(--danger-400);font-style:italic;">— kosong</span>`,a=e=>e?`✅`:`❌`,o=e=>e?new Date(e).toLocaleDateString(`id-ID`,{day:`numeric`,month:`long`,year:`numeric`}):`-`,s=[`nik`,`nama`,`email`,`telepon_1`,`prodi_pilihan`,`asal_sekolah`,`alamat`,`tempat_lahir`,`tanggal_lahir`,`gender`],c=s.filter(t=>e[t]&&String(e[t]).trim()).length,l=Math.round(c/s.length*100),u=l===100?`hsl(145 60% 45%)`:l>=70?`hsl(38 90% 50%)`:`hsl(0 70% 55%)`;n.innerHTML=`
...
`),n=new Blob([t],{type:`text/csv`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=`PMB_Data_${new Date().toISOString().slice(0,10)}.csv`,i.click(),URL.revokeObjectURL(r)}async function Tt(e,t){try{let n,r;switch(e){case`create-account`:if(n=await fetch(`${R}/account/create`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({registration_id:parseInt(t.id)})}),r=await n.json(),n.ok){let e=r.password;alert(e?`✅ Akun dibuat!\n\nNIM: ${r.nim}\nPassword: ${e}\nEmail: ${r.email}`:`ℹ️ Akun sudah dibuat sebelumnya.\n\nNIM: ${r.nim}\nEmail: ${r.email}\n\nPassword hanya ditampilkan saat pertama kali dibuat.`)}else alert(`❌ `+(r.error||`Gagal membuat akun`));break;case`validate`:let e=await fetch(`${R}/account/${t.id}`);if(!e.ok){alert(`❌ Akun belum dibuat. Buat akun terlebih dahulu.`);return}if((await e.json()).is_validated){alert(`ℹ️ Akun sudah divalidasi sebelumnya.`);return}let i=await fetch(`${R}/account/${t.id}/validate`,{method:`PUT`});r=await i.json(),i.ok?(alert(`✅ `+r.message),B()):alert(`❌ `+(r.error||`Gagal validasi`));break;case`confirm-pay`:let a=await fetch(`${R}/payment`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({registration_id:parseInt(t.id),metode_bayar:`cash`,jumlah:35e4})});if(a.ok){let e=await a.json();(await fetch(`${R}/payment/${e.id}/confirm`,{method:`PUT`})).ok&&(alert(`✅ Pembayaran cash dikonfirmasi!`),B())}else{let e=await a.json();e.error&&e.error.includes(`sudah`)?alert(`ℹ️ Pembayaran sudah tercatat.`):alert(`❌ `+(e.error||`Gagal`))}break;case`edit`:Dt(parseInt(t.id));return;case`delete`:let o=z.find(e=>e.id===parseInt(t.id));if(!o||!confirm(`⚠️ Hapus data pendaftaran?\n\n${o.nama} (${o.no_pendaftaran})\n\nSemua data terkait (akun, pembayaran) juga akan dihapus.`))return;n=await fetch(`${R}/registration/${t.id}`,{method:`DELETE`}),r=await n.json(),n.ok?(alert(`✅ `+r.message),B()):alert(`❌ `+(r.error||`Gagal menghapus`));break}}catch(e){alert(`❌ Error: `+e.message)}}function Et(){let e=document.getElementById(`pmbMgmtContent`);e&&(e.innerHTML=`
    <form id="offlineRegForm" style="max-width:640px;">
      ...
      <button type="submit" class="btn btn-primary" id="offlineSubmitBtn" style="width:100%;padding:14px;">
        ✨ Daftarkan & Buat Akun
          ...
          </div>`}else alert(`❌ `+(r.error||`Gagal`)),n.disabled=!1,n.textContent=`📝 Daftarkan & Buat Akun`}catch(e){alert(`❌ `+e.message),n.disabled=!1,n.textContent=`📝 Daftarkan & Buat Akun`}}))}function Dt(e){let t=z.find(t=>t.id===e);if(!t)return;let n=document.getElementById(`pmbDetailModal`),r=document.getElementById(`pmbDetailContent`);if(!n||!r)return;let i=[`Administrasi Negara`,`Administrasi Niaga`],a=e=>e||``;r.innerHTML=`
    ...
    </div>`}function At(){jt(),document.getElementById(`mhsSearch`)?.addEventListener(`input`,Nt),document.getElementById(`mhsFilterProdi`)?.addEventListener(`change`,Nt),document.getElementById(`mhsFilterStatus`)?.addEventListener(`change`,Nt)}async function jt(){try{let e=await fetch(`${Ot}/registrations`);if(!e.ok)throw Error(`Gagal memuat data`);let t=await e.json();W=(Array.isArray(t)?t:t.data||[]).filter(e=>e.status===`diterima`).map((e,t)=>({...e,nim:e.nim||`20260${String(t+1).padStart(4,`0`)}`,angkatan:e.angkatan||(e.created_at?new Date(e.created_at).getFullYear():2026),semester:e.semester||1,status_mhs:e.status_mhs||`aktif`})),Mt(),Pt(W)}catch(e){document.getElementById(`mhsTableContainer`).innerHTML=`
      <div style="text-align:center;padding:32px;">
        <p style="color:var(--text-muted);font-size:var(--text-sm);">⚠️ ${e.message}</p>
      </div>`}}function Mt(){let e=W.length,t=W.filter(e=>e.status_mhs===`aktif`).length,n=W.filter(e=>e.status_mhs===`cuti`).length,r=W.filter(e=>e.status_mhs===`lulus`).length,i=e=>document.getElementById(e);i(`mhsTotal`)&&(i(`mhsTotal`).textContent=e),i(`mhsAktif`)&&(i(`mhsAktif`).textContent=t),i(`mhsCuti`)&&(i(`mhsCuti`).textContent=n),i(`mhsLulus`)&&(i(`mhsLulus`).textContent=r)}function Nt(){let e=(document.getElementById(`mhsSearch`)?.value||``).toLowerCase(),t=document.getElementById(`mhsFilterProdi`)?.value||``,n=document.getElementById(`mhsFilterStatus`)?.value||``;Pt(W.filter(r=>{let i=!e||(r.nama||``).toLowerCase().includes(e)||(r.nim||``).toLowerCase().includes(e)||(r.nik||``).includes(e),a=!t||r.prodi_pilihan===t,o=!n||r.status_mhs===n;return i&&a&&o}))}function Pt(e){let t=document.getElementById(`mhsTableContainer`),n=document.getElementById(`mhsCount`);if(t){if(e.length===0){t.innerHTML=`<div style="text-align:center;padding:32px;">
      <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="var(--gray-300)" stroke-width="1.5" style="margin:0 auto 12px;display:block;"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>
    ...
    </table>`,n&&(n.textContent=`Menampilkan ${e.length} mahasiswa`),t.querySelectorAll(`.mhs-tr`).forEach(e=>{e.addEventListener(`click`,()=>{let t=parseInt(e.dataset.id),n=W.find(e=>e.id===t);n&&Ft(n)})}),t.querySelectorAll(`.mhs-edit-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=parseInt(e.dataset.id),n=W.find(e=>e.id===t);n&&It(n)})}),t.querySelectorAll(`.mhs-del-btn`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=parseInt(e.dataset.id),n=W.find(e=>e.id===t);if(n&&confirm(`⚠️ Hapus data mahasiswa ${n.nama}?\n\nAkun dan data terkait akan dihapus.`))try{let e=await fetch(`${R}/registration/${t}`,{method:`DELETE`});if(e.ok)alert(`✅ Data mahasiswa berhasil dihapus`),jt();else{let t=await e.json();alert(`❌ `+(t.error||`Gagal menghapus`))}}catch(e){alert(`❌ `+e.message)}})})}}function Ft(e){let t=document.getElementById(`mhsModal`),n=document.getElementById(`mhsModalBody`),r=document.getElementById(`mhsModalTitle`);if(!t||!n)return;r&&(r.textContent=`Profil Mahasiswa`);let i=(e,t)=>`
  ...
  `,t.classList.add(`active`)}function It(e){let t=document.getElementById(`mhsModal`),n=document.getElementById(`mhsModalBody`),r=document.getElementById(`mhsModalTitle`);if(!t||!n)return;r&&(r.textContent=`Edit Data Mahasiswa`);let i=[`Administrasi Negara`,`Administrasi Niaga`],a=e=>e||``;n.innerHTML=`
...
Data yang dihapus tidak dapat dikembalikan.`))return;let a=l.findIndex(e=>e.id===r);a>=0&&(l.splice(a,1),alert(`✅ Dosen berhasil dihapus`),e.innerHTML=Ht()+t,Ut(e,t))})})}var Wt={mahasiswa:Se,dosen:Me,kaprodi:qe,bap:Je};function Gt(e){let t=An();if(!t){window.location.hash=`#/login`;return}let n=Wt[t.role]||Se;e.innerHTML=`
    ...
    <main class="portal-main">
      ...
      <div class="portal-welcome">
        <div class="portal-welcome-text">
          <h2>${Kt()}, ${t.nama.split(` `)[0]}</h2>
          <p>${qt()} &nbsp;•&nbsp; <span class="portal-clock-icon">${G.clock}</span> <span id="portalClock">${Jt()}</span></p>
  ...
  <aside class="dash-sidebar elearning-sidebar" id="dashSidebar" role="navigation" aria-label="Menu navigasi E-Learning">
    ...
    <div class="sidebar-user" role="status" aria-label="Informasi pengguna aktif">
      ...
      <div>
        <div class="sidebar-user-name">${e.nama}</div>
        <div class="sidebar-user-role">${$t[e.role]}${e.nim?` — `+e.nim:``}</div>
      </div>
    </div>
    ${t.map(e=>`
  ...
  <header class="dash-topbar" role="banner" aria-label="Header E-Learning">
    ...
    </div>`}function dn(e,t){let n=Y[e.judul]||ln(e.judul,e.soal),r=n.length;return`
    ...
    </div>`}var gn={home:null,kelas:hn};function _n(e,t,n){if(n&&n.activeForum)return pn(n.activeForum.forum,n.activeForum.classId,t.nama||`User`,t.role);if(n&&n.quizIntro)return un(n.quizIntro);if(n&&n.quizResult){let e=n.quizResult;return fn(e.quiz,e.score,e.total,e.answers,e.questions)}if(n&&n.activeQuiz)return dn(n.activeQuiz,n.kelas.id);if(n&&n.kelas)return mn(n.kelas,n.tab,t.role,t.nim);let r=sn[t.role]||rn,i=gn[e];return i?i(t):r(t)}function vn(e){let t=An();if(!t){window.location.hash=`#/login`;return}let n=`home`,r=null,i={},a=null;function o(){let e=document.getElementById(`dashMain`);e&&(e.innerHTML=_n(n,t,r)+`
      <footer class="dash-iso-footer" role="contentinfo" aria-label="Sertifikasi ISO">
    ...
    <div class="pmb-form-container">
      ...
      <form id="pmbForm" class="pmb-form">
        ...
        <div class="pmb-section">
          ...
          <div class="pmb-row">
            ${Q(`Pilih Program Studi *`,`prodi_pilihan`,`text`,!0,yn)}
          </div>
        </div>
    ...
    <div class="pmb-form-container">
      ...
      <div class="pmb-info-cards">
        ...
        <div class="pmb-info-card">
          <h3>🎓 Program Studi</h3>
          <ul class="pmb-req-list">
            ${yn.map(e=>`<li>${e}</li>`).join(``)}
          </ul>
        </div>
      </div>
    </div>`}async function On(e){let t={total_pendaftar:0,total_proses:0,total_diterima:0,total_ditolak:0};try{let e=await fetch(`${X}/stats`);e.ok&&(t=await e.json())}catch(e){console.warn(`Stats fetch failed:`,e)}function n(e){let t=document.getElementById(`pmbMainArea`);if(t){switch(e){case`info`:t.innerHTML=Dn();break;case`daftar`:t.innerHTML=wn();break;case`status`:t.innerHTML=Tn();break;case`bayar`:t.innerHTML=En();break}document.querySelectorAll(`.pmb-nav-link`).forEach(t=>{t.classList.toggle(`active`,t.dataset.pmbPage===e)}),r(e)}}e.innerHTML=`
    <div class="pmb-page">
      <!-- Header -->
        ...
        <span>${Z.checkCircle} ISO 40500 — Accessibility</span>
          ...
          <div class="pmb-payment-card">
            ...
            `}
                      ...
                      </div>`;try{let e=await fetch(`${X}/stats`);e.ok&&(t=await e.json(),document.querySelector(`.pmb-stats`).outerHTML=Sn(t))}catch{}}else alert(`❌ `+(o.error||`Gagal memproses pembayaran`)),e.disabled=!1,e.querySelector(`.pmb-pay-option-title`).textContent=n===`online`?`Bayar Online`:`Bayar Cash`}catch{alert(`❌ Gagal menghubungi server`),e.disabled=!1}})})}catch{r.innerHTML=`<div class="pmb-error">❌ Gagal menghubungi server</div>`}}e.addEventListener(`click`,r),n.addEventListener(`keydown`,e=>{e.key===`Enter`&&r()})}}var $={user:null,role:null,theme:localStorage.getItem(`theme`)||`light`};$.theme===`dark`&&document.documentElement.setAttribute(`data-theme`,`dark`);function kn(e){$.user=e,$.role=e?.role||null,sessionStorage.setItem(`user`,JSON.stringify(e))}function An(){if(!$.user){let e=sessionStorage.getItem(`user`);e&&($.user=JSON.parse(e),$.role=$.user?.role||null)}return $.user}function jn(){$.user=null,$.role=null,sessionStorage.removeItem(`user`),i(`#/login`)}r(`/`,e=>re(e),`Beranda`),r(`/login`,e=>le(e),`Login`),r(`/portal`,e=>Zt(e),`Portal`),r(`/dashboard`,e=>Gt(e),`SIAKAD`),r(`/elearning`,e=>vn(e),`E-Learning`),r(`/pmb`,e=>On(e),`PMB`),document.addEventListener(`DOMContentLoaded`,()=>{a(`app`)});