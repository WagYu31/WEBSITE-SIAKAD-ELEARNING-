...
Rp 350.000`),B()):alert(`❌ Gagal konfirmasi pembayaran`);else{let e=o.snap_token,t=o.snap_url||`https://app.sandbox.midtrans.com/snap/v4/redirection/${e}`;if(!e){alert(`⚠️ Snap token gagal dibuat.

Detail: `+(o.error_detail||o.error||`Unknown`));break}let n=document.createElement(`div`);n.id=`midtrans-modal`,n.style.cssText=`position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);z-index:99999;display:flex;align-items:center;justify-content:center;`,n.innerHTML=`
            <div style="background:white;border-radius:16px;width:min(480px,95vw);height:min(680px,90vh);display:flex;flex-direction:column;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,0.4);">
              ...
              <iframe src="${t}" style="flex:1;border:none;width:100%;"></iframe>
            </div>
          `,document.body.appendChild(n),document.getElementById(`close-midtrans-modal`).onclick=()=>{n.remove(),B()},n.addEventListener(`click`,e=>{e.target===n&&(n.remove(),B())})}break}case`edit`:Rt(parseInt(t.id));return;case`delete`:let a=z.find(e=>e.id===parseInt(t.id));if(!a||!confirm(`⚠️ Hapus data pendaftaran?\n\n${a.nama} (${a.no_pendaftaran})\n\nSemua data terkait (akun, pembayaran) juga akan dihapus.`))return;n=await fetch(`${R}/registration/${t.id}`,{method:`DELETE`}),r=await n.json(),n.ok?(alert(`✅ `+r.message),B()):alert(`❌ `+(r.error||`Gagal menghapus`));break}}catch(e){alert(`❌ Error: `+e.message)}}function Lt(){let e=document.getElementById(`pmbMgmtContent`);e&&(e.innerHTML=`
