...
Data yang dihapus tidak dapat dikembalikan.`))return;let a=l.findIndex(e=>e.id===r);a>=0&&(l.splice(a,1),alert(`✅ Dosen berhasil dihapus`),e.innerHTML=en()+t,tn(e,t))})})}var nn={mahasiswa:we,dosen:Pe,kaprodi:Je,bap:Ye};function rn(e){let t=Gn();if(!t){window.location.hash=`#/login`;return}let n=nn[t.role]||we;if(e.innerHTML=`
  ...
  <header class="dash-topbar" role="banner" aria-label="Header E-Learning">
    ...
    </div>`}var An={home:null,kelas:kn};function jn(e,t,n){if(n&&n.activeForum)return Dn(n.activeForum.forum,n.activeForum.classId,t.nama||`User`,t.role);if(n&&n.quizIntro)return wn(n.quizIntro);if(n&&n.quizResult){let e=n.quizResult;return En(e.quiz,e.score,e.total,e.answers,e.questions)}if(n&&n.activeQuiz)return Tn(n.activeQuiz,n.kelas.id);if(n&&n.kelas)return On(n.kelas,n.tab,t.role,t.nim);let r=xn[t.role]||_n,i=An[e];return i?i(t):r(t)}async function Mn(e){let t=Gn();if(!t){window.location.hash=`#/login`;return}if(t.role===`dosen`&&t.nip)try{let e=await fetch(`/api/dosen/jadwal-mengajar/${encodeURIComponent(t.nip)}`);if(e.ok){let n=await e.json(),r=n.data||n.jadwal||n||[],i=new Set;vn=r.reduce((e,n)=>{let r=n.kode_mk||n.mata_kuliah_id||n.mata_kuliah;return r&&!i.has(r)&&(i.add(r),e.push({id:n.mata_kuliah_id||n.id||e.length+100,kode:n.kode_mk||`-`,nama:n.mata_kuliah||n.nama_mk||`-`,dosen:t.nama,sks:n.sks||3,semester:n.tahun_ajaran||`Genap 2025/2026`,kelas:n.kelas||`A`,hari:n.hari||`-`,jadwal:(n.hari||`-`)+`, `+(n.jam||`-`),ruang:n.ruang||`-`,mahasiswa:n.jumlah_mahasiswa||30,progress:50,totalMateri:14,materiSelesai:7})),e},[])}}catch(e){console.warn(`Gagal fetch jadwal dosen:`,e)}let n=`home`,r=null,i={},a=null;function o(){let e=document.getElementById(`dashMain`);e&&(e.innerHTML=jn(n,t,r)+`
      <footer class="dash-iso-footer" role="contentinfo" aria-label="Sertifikasi ISO">
