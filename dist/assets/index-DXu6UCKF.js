...
Setelah mahasiswa bayar, status akan otomatis terupdate.`)):o.snap_token?window.open(`https://app.sandbox.midtrans.com/snap/v2/vtweb/${o.snap_token}`,`_blank`):alert(`⚠️ Snap token gagal dibuat.

Detail: `+(o.error_detail||o.error||`Unknown error`)+`

Coba buka: /api/pmb/payment/test untuk debug.`);break}case`edit`:Rt(parseInt(t.id));return;case`delete`:let a=z.find(e=>e.id===parseInt(t.id));if(!a||!confirm(`⚠️ Hapus data pendaftaran?\n\n${a.nama} (${a.no_pendaftaran})\n\nSemua data terkait (akun, pembayaran) juga akan dihapus.`))return;n=await fetch(`${R}/registration/${t.id}`,{method:`DELETE`}),r=await n.json(),n.ok?(alert(`✅ `+r.message),B()):alert(`❌ `+(r.error||`Gagal menghapus`));break}}catch(e){alert(`❌ Error: `+e.message)}}function Lt(){let e=document.getElementById(`pmbMgmtContent`);e&&(e.innerHTML=`
