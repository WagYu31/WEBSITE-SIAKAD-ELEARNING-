...
Data yang dihapus tidak dapat dikembalikan.`))return;let a=l.findIndex(e=>e.id===r);a>=0&&(l.splice(a,1),alert(`✅ Dosen berhasil dihapus`),e.innerHTML=en()+t,tn(e,t))})})}var nn={mahasiswa:we,dosen:Pe,kaprodi:Je,bap:Ye};function rn(e){let t=Gn();if(!t){window.location.hash=`#/login`;return}let n=nn[t.role]||we;if(e.innerHTML=`
  ...
  <header class="dash-topbar" role="banner" aria-label="Header E-Learning">
    ...
    </div>`}var vn=[];function yn(e){let t=vn.length>0?vn:b,n=t.slice(0,4);return`
    <div class="stat-grid">
      <div class="stat-box">
    ...
    </div>`}var An={home:null,kelas:kn};function jn(e,t,n){if(n&&n.activeForum)return Dn(n.activeForum.forum,n.activeForum.classId,t.nama||`User`,t.role);if(n&&n.quizIntro)return wn(n.quizIntro);if(n&&n.quizResult){let e=n.quizResult;return En(e.quiz,e.score,e.total,e.answers,e.questions)}if(n&&n.activeQuiz)return Tn(n.activeQuiz,n.kelas.id);if(n&&n.kelas)return On(n.kelas,n.tab,t.role,t.nim);let r=xn[t.role]||_n,i=An[e];return i?i(t):r(t)}async function Mn(e){let t=Gn();if(!t){window.location.hash=`#/login`;return}if(t.role===`dosen`){let e=[];try{if(t.nip){let n=await fetch(`/api/dosen/jadwal-mengajar/${encodeURIComponent(t.nip)}`);if(n.ok){let r=await n.json(),i=Array.isArray(r.data)?r.data:[],a=new Set;i.forEach(n=>{let r=n.mata_kuliah||{},i=r.id||n.mata_kuliah_id;if(i&&!a.has(i)){a.add(i);let o=(n.waktu||``).split(`,`),s=(o[0]||`-`).trim(),c=(o[1]||`-`).trim();e.push({id:r.id||n.mata_kuliah_id||e.length+100,kode:r.kode||`-`,nama:r.nama||`-`,dosen:t.nama,sks:r.sks||n.sks||3,semester:n.tahun_ajaran||`Genap 2025/2026`,kelas:n.kelas||`A`,hari:s,jam:c,jadwal:n.waktu||s+`, `+c,ruang:n.ruang||`-`,mahasiswa:n.jumlah_mhs||30,progress:50,totalMateri:14,materiSelesai:7})}})}}}catch(e){console.warn(`Gagal fetch jadwal dosen:`,e)}if(e.length===0){let n=l.find(e=>e.nip===t.nip||e.nama===t.nama);if(n&&n.mataKuliah&&n.mataKuliah.length>0){let r=[`Senin`,`Selasa`,`Rabu`,`Kamis`,`Jumat`],i=[`07:30-09:10`,`09:20-11:00`,`13:00-14:40`,`14:50-16:30`];e=n.mataKuliah.map((e,n)=>({id:100+n,kode:`MK`+String(101+n).padStart(3,`0`),nama:e,dosen:t.nama,sks:3,semester:`Genap 2025/2026`,kelas:n%2==0?`A`:`B`,hari:r[n%r.length],jam:i[n%i.length],jadwal:r[n%r.length]+`, `+i[n%i.length],ruang:`R.`+(201+n),mahasiswa:25+n*3,progress:50,totalMateri:14,materiSelesai:7}))}}e.length>0&&(vn=e)}let n=`home`,r=null,i={},a=null;function o(){let e=document.getElementById(`dashMain`);e&&(e.innerHTML=jn(n,t,r)+`
      <footer class="dash-iso-footer" role="contentinfo" aria-label="Sertifikasi ISO">
