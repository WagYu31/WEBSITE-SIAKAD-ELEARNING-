(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e={},t={},n=null;function r(n,r,i=``){e[n]=r,t[n]=i}function i(e){window.location.hash=e}function a(e){n=document.getElementById(e),window.addEventListener(`hashchange`,o),window.addEventListener(`load`,o),window.location.hash?o():window.location.hash=`#/`}async function o(){let r=window.location.hash.slice(1)||`/`,i=e[r];if(!i){for(let[t,n]of Object.entries(e))if(t.includes(`:`)){let e=RegExp(`^`+t.replace(/:[^/]+/g,`([^/]+)`)+`$`);if(r.match(e)){i=n;break}}}if(i&&n){n.style.opacity=`0`,n.style.transform=`translateY(10px)`,await new Promise(e=>setTimeout(e,150));try{await i(n)}catch(e){console.error(`Route error:`,e),n.innerHTML=`
  ...
  <header class="dash-topbar" role="banner" aria-label="Header SIAKAD">
    ...
    </div>`}async function Ct(){document.querySelectorAll(`.pmb-mgmt-btn`).forEach(e=>{e.addEventListener(`click`,()=>{document.querySelectorAll(`.pmb-mgmt-btn`).forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),e.dataset.tab===`list`?B():e.dataset.tab===`add`&&Lt()})}),document.getElementById(`closeDetailModal`)?.addEventListener(`click`,()=>{document.getElementById(`pmbDetailModal`).style.display=`none`}),document.getElementById(`pmbDetailModal`)?.addEventListener(`click`,e=>{e.target.id===`pmbDetailModal`&&(e.target.style.display=`none`)}),B()}var z=[];async function B(){let e=document.getElementById(`pmbMgmtContent`);if(e){e.innerHTML=`<div style="text-align:center;padding:24px;">
  ...
  </div>`;try{let[e,t]=await Promise.all([fetch(`${R}/registrations`),fetch(`${R}/stats`)]),n=await e.json(),r=await t.json();z=n.data||[],Tt(r,z)}catch{e.innerHTML=`<div style="text-align:center;padding:24px;">
    ...
    </div>`}}}var V=1,wt=20,H={key:`created_at`,dir:`desc`},U=new Set;function Tt(e,t){let n=document.getElementById(`pmbMgmtContent`);if(!n)return;n.innerHTML=`
    <!-- Stat Cards -->
    ...
    </div>`;let r=()=>{V=1,Et()};document.getElementById(`pmbSearch`)?.addEventListener(`input`,r),document.getElementById(`pmbFilterStatus`)?.addEventListener(`change`,r),document.getElementById(`pmbFilterProdi`)?.addEventListener(`change`,r),document.getElementById(`pmbDateFrom`)?.addEventListener(`change`,r),document.getElementById(`pmbDateTo`)?.addEventListener(`change`,r),document.getElementById(`pmbClearDate`)?.addEventListener(`click`,()=>{document.getElementById(`pmbDateFrom`).value=``,document.getElementById(`pmbDateTo`).value=``,r()}),document.getElementById(`pmbExportBtn`)?.addEventListener(`click`,()=>Ft(z)),document.getElementById(`pmbBulkValidate`)?.addEventListener(`click`,At),document.getElementById(`pmbBulkAccount`)?.addEventListener(`click`,jt),document.getElementById(`pmbBulkDelete`)?.addEventListener(`click`,Mt),document.getElementById(`pmbBulkCancel`)?.addEventListener(`click`,()=>{U.clear(),kt(),Et()}),Et()}function Et(){let e=(document.getElementById(`pmbSearch`)?.value||``).toLowerCase(),t=document.getElementById(`pmbFilterStatus`)?.value||``,n=document.getElementById(`pmbFilterProdi`)?.value||``,r=document.getElementById(`pmbDateFrom`)?.value||``,i=document.getElementById(`pmbDateTo`)?.value||``,a=z.filter(a=>{let o=!e||(a.nama||``).toLowerCase().includes(e)||(a.nik||``).includes(e)||(a.no_pendaftaran||``).toLowerCase().includes(e),s=!t||a.status===t,c=!n||a.prodi_pilihan===n,l=!0;if(r||i){let e=a.created_at?a.created_at.slice(0,10):``;r&&e<r&&(l=!1),i&&e>i&&(l=!1)}return o&&s&&c&&l}),{key:o,dir:s}=H;a.sort((e,t)=>{let n=e[o]||``,r=t[o]||``;return typeof n==`string`&&(n=n.toLowerCase()),typeof r==`string`&&(r=r.toLowerCase()),n<r?s===`asc`?-1:1:n>r?s===`asc`?1:-1:0});let c=a.length,l=Math.max(1,Math.ceil(c/wt));V>l&&(V=l);let u=(V-1)*wt,d=a.slice(u,u+wt);document.getElementById(`pmbTableWrapper`).innerHTML=Ot(d),document.getElementById(`pmbResultCount`).textContent=c>0?`Menampilkan ${u+1}–${Math.min(u+wt,c)} dari ${c} pendaftar`:`Tidak ada data`,Dt(l),Nt()}function Dt(e){let t=document.getElementById(`pmbPagination`);if(!t||e<=1){t&&(t.innerHTML=``);return}let n=e=>`style="padding:4px 10px;border-radius:6px;border:1px solid ${e?`var(--primary-500)`:`var(--gray-200)`};background:${e?`var(--primary-500)`:`#fff`};color:${e?`#fff`:`var(--text-primary)`};cursor:pointer;font-size:0.75rem;font-weight:600;"`,r=`<button ${n(!1)} ${V<=1?`disabled`:``} data-pg="${V-1}">‹</button>`,i=Math.max(1,V-2),a=Math.min(e,i+4);a-i<4&&(i=Math.max(1,a-4));for(let e=i;e<=a;e++)r+=`<button ${n(e===V)} data-pg="${e}">${e}</button>`;r+=`<button ${n(!1)} ${V>=e?`disabled`:``} data-pg="${V+1}">›</button>`,t.innerHTML=r,t.querySelectorAll(`button[data-pg]`).forEach(t=>t.addEventListener(`click`,()=>{let n=parseInt(t.dataset.pg);n>=1&&n<=e&&(V=n,Et())}))}function Ot(e){if(e.length===0)return`<div style="text-align:center;padding:32px;">
        ...
        ${e.map(e=>`<tr class="pmb-tr" data-id="${e.id}" style="cursor:pointer;${U.has(e.id)?`background:hsl(215 80% 97%);`:``}">
          ...
          <td onclick="event.stopPropagation();">
            <div style="display:flex;gap:4px;white-space:nowrap;flex-wrap:wrap;">
              <button class="mgmt-action-btn" data-action="view" data-id="${e.id}" title="Lihat Kelengkapan Data" style="color:hsl(210 60% 50%);">👁️</button>
    ...
    </table>`}function kt(){let e=document.getElementById(`pmbBulkBar`),t=document.getElementById(`pmbBulkCount`);e&&(U.size>0?(e.style.display=`flex`,t&&(t.textContent=`${U.size} pendaftar dipilih`)):e.style.display=`none`)}async function At(){if(!confirm(`Validasi ${U.size} pendaftar sekaligus?`))return;let e=0,t=0;for(let n of U)try{(await fetch(`${R}/account/${n}/validate`,{method:`PUT`})).ok?e++:t++}catch{t++}alert(`✅ Berhasil: ${e}\n❌ Gagal: ${t}`),U.clear(),B()}async function jt(){if(!confirm(`Buat akun untuk ${U.size} pendaftar sekaligus?`))return;let e=0,t=0;for(let n of U)try{(await fetch(`${R}/account/create`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({registration_id:n})})).ok?e++:t++}catch{t++}alert(`✅ Berhasil: ${e}\n❌ Gagal: ${t}`),U.clear(),B()}async function Mt(){if(!confirm(`⚠️ HAPUS ${U.size} pendaftar?\n\nAksi ini tidak bisa dibatalkan!`))return;let e=0,t=0;for(let n of U)try{(await fetch(`${R}/registration/${n}`,{method:`DELETE`})).ok?e++:t++}catch{t++}alert(`✅ Terhapus: ${e}\n❌ Gagal: ${t}`),U.clear(),B()}function Nt(){document.querySelectorAll(`.mgmt-action-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation(),It(e.dataset.action,e.dataset)})}),document.querySelectorAll(`.pmb-tr`).forEach(e=>{e.addEventListener(`click`,()=>{let t=parseInt(e.dataset.id),n=z.find(e=>e.id===t);n&&Pt(n)})}),document.querySelectorAll(`.sortable-th`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.sort;H.key===t?H.dir=H.dir===`asc`?`desc`:`asc`:H={key:t,dir:`asc`},Et()})}),document.getElementById(`pmbSelectAll`)?.addEventListener(`change`,e=>{document.querySelectorAll(`.pmb-chk`).forEach(t=>{let n=parseInt(t.dataset.id);e.target.checked?U.add(n):U.delete(n),t.checked=e.target.checked}),kt()}),document.querySelectorAll(`.pmb-chk`).forEach(e=>{e.addEventListener(`change`,()=>{let t=parseInt(e.dataset.id);e.checked?U.add(t):U.delete(t),kt()})})}function Pt(e){let t=document.getElementById(`pmbDetailModal`),n=document.getElementById(`pmbDetailContent`);if(!t||!n)return;let r={diterima:`badge-success`,ditolak:`badge-danger`,proses:`badge-primary`,menunggu:`badge-warning`},i=e=>e||`<span style="color:var(--danger-400);font-style:italic;">— kosong</span>`,a=e=>e?`✅`:`❌`,o=e=>e?new Date(e).toLocaleDateString(`id-ID`,{day:`numeric`,month:`long`,year:`numeric`}):`-`,s=[`nik`,`nama`,`email`,`telepon_1`,`prodi_pilihan`,`asal_sekolah`,`alamat`,`tempat_lahir`,`tanggal_lahir`,`gender`],c=s.filter(t=>e[t]&&String(e[t]).trim()).length,l=Math.round(c/s.length*100),u=l===100?`hsl(145 60% 45%)`:l>=70?`hsl(38 90% 50%)`:`hsl(0 70% 55%)`;n.innerHTML=`
    ...
    </div>`,n.querySelectorAll(`.mgmt-action-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation(),It(e.dataset.action,e.dataset)})}),t.style.display=`flex`}function Ft(e){if(!e.length){alert(`Tidak ada data untuk diexport.`);return}let t=[[`No Pendaftaran`,`Nama`,`NIK`,`Email`,`Prodi`,`Metode`,`Status`,`Asal Sekolah`,`Telepon`],...e.map(e=>[e.no_pendaftaran,e.nama,e.nik,e.email||``,e.prodi_pilihan||``,e.metode,e.status,e.asal_sekolah||``,e.telepon_1||``])].map(e=>e.map(e=>`"${e}"`).join(`,`)).join(`
`),n=new Blob([t],{type:`text/csv`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=`PMB_Data_${new Date().toISOString().slice(0,10)}.csv`,i.click(),URL.revokeObjectURL(r)}async function It(e,t){try{let n,r;switch(e){case`view`:{let e=z.find(e=>e.id===parseInt(t.id));e&&Pt(e);return}case`create-account`:if(n=await fetch(`${R}/account/create`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({registration_id:parseInt(t.id)})}),r=await n.json(),n.ok){let e=r.password;alert(e?`✅ Akun dibuat!\n\nNIM: ${r.nim}\nPassword: ${e}\nEmail: ${r.email}`:`ℹ️ Akun sudah dibuat sebelumnya.\n\nNIM: ${r.nim}\nEmail: ${r.email}\n\nPassword hanya ditampilkan saat pertama kali dibuat.`)}else alert(`❌ `+(r.error||`Gagal membuat akun`));break;case`validate`:let e=await fetch(`${R}/account/${t.id}`);if(!e.ok){alert(`❌ Akun belum dibuat. Buat akun terlebih dahulu.`);return}if((await e.json()).is_validated){alert(`ℹ️ Akun sudah divalidasi sebelumnya.`);return}let i=await fetch(`${R}/account/${t.id}/validate`,{method:`PUT`});r=await i.json(),i.ok?(alert(`✅ `+r.message),B()):alert(`❌ `+(r.error||`Gagal validasi`));break;case`confirm-pay`:{let e=z.find(e=>e.id===parseInt(t.id)),n=e?e.nama:`Pendaftar`,r=prompt(`💰 Pembayaran untuk: ${n}\n\nPilih metode:\n1 = Cash (langsung konfirmasi)\n2 = Online (Midtrans)\n\nKetik 1 atau 2:`);if(!r||![`1`,`2`].includes(r.trim())){alert(`Dibatalkan`);return}let i=r.trim()===`1`?`cash`:`online`,a=await fetch(`${R}/payment`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({registration_id:parseInt(t.id),metode_bayar:i,jumlah:35e4})}),o=await a.json();if(!a.ok){o.error&&o.error.includes(`sudah lunas`)?alert(`ℹ️ Pembayaran sudah lunas.`):o.snap_token?window.open(o.snap_url||`https://app.sandbox.midtrans.com/snap/v2/vtweb/${o.snap_token}`,`_blank`):alert(`❌ `+(o.error||`Gagal`));return}i===`cash`?(await fetch(`${R}/payment/${o.id}/confirm`,{method:`PUT`})).ok?(alert(`✅ Pembayaran cash dikonfirmasi!
Rp 350.000`),B()):alert(`❌ Gagal konfirmasi pembayaran`):o.snap_url?(window.open(o.snap_url,`_blank`),alert(`🔗 Halaman pembayaran Midtrans sudah dibuka di tab baru.

Setelah mahasiswa bayar, status akan otomatis terupdate.`)):o.snap_token?window.open(`https://app.sandbox.midtrans.com/snap/v2/vtweb/${o.snap_token}`,`_blank`):alert(`⚠️ Snap token gagal dibuat. Cek konfigurasi Midtrans.`);break}case`edit`:Rt(parseInt(t.id));return;case`delete`:let a=z.find(e=>e.id===parseInt(t.id));if(!a||!confirm(`⚠️ Hapus data pendaftaran?\n\n${a.nama} (${a.no_pendaftaran})\n\nSemua data terkait (akun, pembayaran) juga akan dihapus.`))return;n=await fetch(`${R}/registration/${t.id}`,{method:`DELETE`}),r=await n.json(),n.ok?(alert(`✅ `+r.message),B()):alert(`❌ `+(r.error||`Gagal menghapus`));break}}catch(e){alert(`❌ Error: `+e.message)}}function Lt(){let e=document.getElementById(`pmbMgmtContent`);e&&(e.innerHTML=`
    <form id="offlineRegForm" style="max-width:640px;">
      ...
      <button type="submit" class="btn btn-primary" id="offlineSubmitBtn" style="width:100%;padding:14px;">
        ✨ Daftarkan & Buat Akun
          ...
          </div>`}else alert(`❌ `+(r.error||`Gagal`)),n.disabled=!1,n.textContent=`📝 Daftarkan & Buat Akun`}catch(e){alert(`❌ `+e.message),n.disabled=!1,n.textContent=`📝 Daftarkan & Buat Akun`}}))}function Rt(e){let t=z.find(t=>t.id===e);if(!t)return;let n=document.getElementById(`pmbDetailModal`),r=document.getElementById(`pmbDetailContent`);if(!n||!r)return;let i=[`Administrasi Negara`,`Administrasi Niaga`],a=e=>e||``;r.innerHTML=`
    ...
    </div>`}function Vt(){Ht(),document.getElementById(`mhsSearch`)?.addEventListener(`input`,Wt),document.getElementById(`mhsFilterProdi`)?.addEventListener(`change`,Wt),document.getElementById(`mhsFilterStatus`)?.addEventListener(`change`,Wt)}async function Ht(){try{let e=await fetch(`${zt}/registrations`);if(!e.ok)throw Error(`Gagal memuat data`);let t=await e.json();W=(Array.isArray(t)?t:t.data||[]).filter(e=>e.status===`diterima`).map((e,t)=>({...e,nim:e.nim||`20260${String(t+1).padStart(4,`0`)}`,angkatan:e.angkatan||(e.created_at?new Date(e.created_at).getFullYear():2026),semester:e.semester||1,status_mhs:e.status_mhs||`aktif`})),W.length===0&&(W=te.map(e=>({...e}))),Ut(),Gt(W)}catch{W=te.map(e=>({...e})),Ut(),Gt(W)}}function Ut(){let e=W.length,t=W.filter(e=>e.status_mhs===`aktif`).length,n=W.filter(e=>e.status_mhs===`cuti`).length,r=W.filter(e=>e.status_mhs===`lulus`).length,i=e=>document.getElementById(e);i(`mhsTotal`)&&(i(`mhsTotal`).textContent=e),i(`mhsAktif`)&&(i(`mhsAktif`).textContent=t),i(`mhsCuti`)&&(i(`mhsCuti`).textContent=n),i(`mhsLulus`)&&(i(`mhsLulus`).textContent=r)}function Wt(){let e=(document.getElementById(`mhsSearch`)?.value||``).toLowerCase(),t=document.getElementById(`mhsFilterProdi`)?.value||``,n=document.getElementById(`mhsFilterStatus`)?.value||``;Gt(W.filter(r=>{let i=!e||(r.nama||``).toLowerCase().includes(e)||(r.nim||``).toLowerCase().includes(e)||(r.nik||``).includes(e),a=!t||r.prodi_pilihan===t,o=!n||r.status_mhs===n;return i&&a&&o}))}function Gt(e){let t=document.getElementById(`mhsTableContainer`),n=document.getElementById(`mhsCount`);if(t){if(e.length===0){t.innerHTML=`<div style="text-align:center;padding:32px;">
      <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="var(--gray-300)" stroke-width="1.5" style="margin:0 auto 12px;display:block;"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>
  ...
  `,t.style.display=`flex`}function qt(e){let t=document.getElementById(`mhsModal`),n=document.getElementById(`mhsModalBody`),r=document.getElementById(`mhsModalTitle`);if(!t||!n)return;r&&(r.textContent=`Edit Data Mahasiswa`);let i=[`Administrasi Negara`,`Administrasi Niaga`],a=e=>e||``;n.innerHTML=`
...
Data yang dihapus tidak dapat dikembalikan.`))return;let a=l.findIndex(e=>e.id===r);a>=0&&(l.splice(a,1),alert(`✅ Dosen berhasil dihapus`),e.innerHTML=$t()+t,en(e,t))})})}var tn={mahasiswa:Te,dosen:Fe,kaprodi:Ye,bap:Xe};function nn(e){let t=Vn();if(!t){window.location.hash=`#/login`;return}let n=tn[t.role]||Te;e.innerHTML=`
    ...
    <main class="portal-main">
      ...
      <div class="portal-welcome">
        <div class="portal-welcome-text">
          <h2>${rn()}, ${t.nama.split(` `)[0]}</h2>
          <p>${an()} &nbsp;•&nbsp; <span class="portal-clock-icon">${G.clock}</span> <span id="portalClock">${on()}</span></p>
  ...
  <aside class="dash-sidebar elearning-sidebar" id="dashSidebar" role="navigation" aria-label="Menu navigasi E-Learning">
    ...
    <div class="sidebar-user" role="status" aria-label="Informasi pengguna aktif">
      ...
      <div>
        <div class="sidebar-user-name">${e.nama}</div>
        <div class="sidebar-user-role">${dn[e.role]}${e.nim?` — `+e.nim:``}</div>
      </div>
    </div>
    ${t.map(e=>`
  ...
  <header class="dash-topbar" role="banner" aria-label="Header E-Learning">
    ...
    <div class="el-welcome-banner">
      ...
      <div class="el-welcome-text">
        ...
        <h2>${fn()}, ${(e.nama||e.name||`Mahasiswa`).split(` `)[0]}! 👋</h2>
    ...
    </div>`}function Sn(e,t){let n=Y[e.judul]||bn(e.judul,e.soal),r=n.length;return`
    ...
    </div>`}var Dn={home:null,kelas:En};function On(e,t,n){if(n&&n.activeForum)return wn(n.activeForum.forum,n.activeForum.classId,t.nama||`User`,t.role);if(n&&n.quizIntro)return xn(n.quizIntro);if(n&&n.quizResult){let e=n.quizResult;return Cn(e.quiz,e.score,e.total,e.answers,e.questions)}if(n&&n.activeQuiz)return Sn(n.activeQuiz,n.kelas.id);if(n&&n.kelas)return Tn(n.kelas,n.tab,t.role,t.nim);let r=vn[t.role]||hn,i=Dn[e];return i?i(t):r(t)}function kn(e){let t=Vn();if(!t){window.location.hash=`#/login`;return}let n=`home`,r=null,i={},a=null;function o(){let e=document.getElementById(`dashMain`);e&&(e.innerHTML=On(n,t,r)+`
      <footer class="dash-iso-footer" role="contentinfo" aria-label="Sertifikasi ISO">
    ...
    <div class="pmb-form-container">
      ...
      <form id="pmbForm" class="pmb-form">
        ...
        <div class="pmb-section">
          ...
          <div class="pmb-row">
            ${Q(`Pilih Program Studi *`,`prodi_pilihan`,`text`,!0,An)}
          </div>
        </div>
    ...
    <div class="pmb-form-container">
      ...
      <div class="pmb-info-cards">
        ...
        <div class="pmb-info-card">
          <h3>🎓 Program Studi</h3>
          <ul class="pmb-req-list">
            ${An.map(e=>`<li>${e}</li>`).join(``)}
          </ul>
        </div>
      </div>
    </div>`}async function zn(e){let t={total_pendaftar:0,total_proses:0,total_diterima:0,total_ditolak:0};try{let e=await fetch(`${X}/stats`);e.ok&&(t=await e.json())}catch(e){console.warn(`Stats fetch failed:`,e)}function n(e){let t=document.getElementById(`pmbMainArea`);if(t){switch(e){case`info`:t.innerHTML=Rn();break;case`daftar`:t.innerHTML=Fn();break;case`status`:t.innerHTML=In();break;case`bayar`:t.innerHTML=Ln();break}document.querySelectorAll(`.pmb-nav-link`).forEach(t=>{t.classList.toggle(`active`,t.dataset.pmbPage===e)}),r(e)}}e.innerHTML=`
    <div class="pmb-page">
      <!-- Header -->
        ...
        <span>${Z.checkCircle} ISO 40500 — Accessibility</span>
          ...
          <div class="pmb-payment-card">
            ...
            `}
                      ...
                      </div>`;try{let e=await fetch(`${X}/stats`);e.ok&&(t=await e.json(),document.querySelector(`.pmb-stats`).outerHTML=Nn(t))}catch{}}else alert(`❌ `+(o.error||`Gagal memproses pembayaran`)),e.disabled=!1,e.querySelector(`.pmb-pay-option-title`).textContent=n===`online`?`Bayar Online`:`Bayar Cash`}catch{alert(`❌ Gagal menghubungi server`),e.disabled=!1}})})}catch{r.innerHTML=`<div class="pmb-error">❌ Gagal menghubungi server</div>`}}e.addEventListener(`click`,r),n.addEventListener(`keydown`,e=>{e.key===`Enter`&&r()})}}var $={user:null,role:null,theme:localStorage.getItem(`theme`)||`light`};$.theme===`dark`&&document.documentElement.setAttribute(`data-theme`,`dark`);function Bn(e){$.user=e,$.role=e?.role||null,sessionStorage.setItem(`user`,JSON.stringify(e))}function Vn(){if(!$.user){let e=sessionStorage.getItem(`user`);e&&($.user=JSON.parse(e),$.role=$.user?.role||null)}return $.user}function Hn(){$.user=null,$.role=null,sessionStorage.removeItem(`user`),i(`#/login`)}r(`/`,e=>oe(e),`Beranda`),r(`/login`,e=>fe(e),`Login`),r(`/portal`,e=>ln(e),`Portal`),r(`/dashboard`,e=>nn(e),`SIAKAD`),r(`/elearning`,e=>kn(e),`E-Learning`),r(`/pmb`,e=>zn(e),`PMB`),document.addEventListener(`DOMContentLoaded`,()=>{a(`app`)});