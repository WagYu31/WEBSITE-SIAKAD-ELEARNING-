    ...
    <div class="dash-card">
      <div class="dash-card-body" style="padding:0;overflow-x:auto;">
        <table class="sch-table" id="jadwalTable" style="width:100%;font-size:0.78rem;">
          ...
          <tbody>
            ${JADWAL_DUMMY.map((e,t)=>{let o=n[e.tipeKelas]||`hsl(215 15% 50%)`,s=r[e.tipeKelas]||`hsl(215 20% 95%)`,c=e.tipeKelas===`Online`?`🌐`:e.tipeKelas===`Hybrid`?`🔄`:`🏫`,l=!!e.gabunganId,u=l?`🔗 Gabungan`:e.prodi===`niaga`?`Niaga`:`Negara`,d=l?`hsl(40 75% 32%)`:i[e.prodi]||`hsl(215 15% 50%)`,f=l?`hsl(40 80% 94%)`:a[e.prodi]||`hsl(215 20% 95%)`;return`<tr data-id="${e.id}" data-prodi="${e.prodi}" data-smt="${e.semester}" data-hari="${e.hari}" data-tipe="${e.tipeKelas}" data-dosen="${liveDosen}" style="${l?`border-left:3px solid hsl(40 70% 60%);`:``}">\n                <td>${t+1}.</td>\n                <td><span style="padding:2px 8px;border-radius:10px;font-size:0.62rem;font-weight:700;background:${f};color:${d};">${u}</span></td>
                ...
                <td colspan="12" style="padding:0;border-top:none;">
                  <div style="background:linear-gradient(180deg,hsl(210 30% 97%),hsl(210 20% 99%));padding:14px 18px;border-top:2px solid hsl(200 55% 75%);">
                    ...
                    <div class="jadwal-pertemuan-grid" data-jid="${e.id}" style="display:grid;grid-template-columns:repeat(14,1fr);gap:4px;">
                      ${pertemuanDates.map((t,n)=>{let r=e.customSchedule&&e.customSchedule[n],i=r&&r.mode||modes[n],a=i===`online`,o=t.getFullYear()+`-`+String(t.getMonth()+1).padStart(2,`0`)+`-`+String(t.getDate()).padStart(2,`0`),s=r&&r.date?r.date:o,c=r&&r.date?new Date(r.date+`T00:00:00`):t,l=r&&r.date&&r.date!==o,u=r&&r.start?r.start:e.jamMulai,d=r&&r.end?r.end:e.jamSelesai,f=r&&r.note?r.note:``,p=l?`linear-gradient(135deg,hsl(40 65% 92%),hsl(40 60% 88%))`:a?`linear-gradient(135deg,hsl(150 50% 92%),hsl(150 45% 88%))`:`linear-gradient(135deg,hsl(210 40% 95%),hsl(210 35% 91%))`,m=l?`hsl(40 65% 65%)`:a?`hsl(150 55% 70%)`:`hsl(210 40% 82%)`,h=l?`🔄`:a?`🌐`:`🏫`,g=l?`Pengganti`:a?`Online`:`Offline`,_=l?`hsl(40 65% 35%)`:a?`hsl(150 55% 35%)`:`hsl(210 45% 40%)`,b=l?`hsl(40 60% 30%)`:`hsl(210 40% 38%)`;return`<div class="prt-mode-card" data-jid="${e.id}" data-pi="${n}" data-mode="${i}" data-orig-date="${o}" data-date="${s}" data-start="${u}" data-end="${d}" data-note="${f.replace(/"/g,`&quot;`)}" style="background:${p};border:1px solid ${m};border-radius:8px;padding:5px 3px;text-align:center;cursor:pointer;transition:all 0.2s ease;user-select:none;position:relative;" title="${y(c)} — ${g} • ${u}-${d}${f?` • 📝 `+f:``} (Klik untuk edit)" onclick="window.__openPertemuanEdit && window.__openPertemuanEdit(this,${e.id},${n})">
                          <div style="font-size:0.5rem;font-weight:800;color:hsl(210 30% 45%);">P${n+1}</div>
                          <div class="prt-date-label" style="font-size:0.58rem;font-weight:700;color:${b};margin:1px 0;">${v(c)}</div>
                          <div class="prt-time-label" style="font-size:0.48rem;color:hsl(215 20% 50%);margin-bottom:1px;">${u}-${d}</div>
                          <div class="prt-icon" style="font-size:0.65rem;">${h}</div>
                          <div class="prt-label" style="font-size:0.46rem;font-weight:700;color:${_};margin-top:1px;">${g}</div>
